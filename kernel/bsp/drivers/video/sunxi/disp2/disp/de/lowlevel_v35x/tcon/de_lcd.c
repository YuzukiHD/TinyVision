...
s32 tcon0_dsi_clk_source(u32 sel, struct disp_panel_para *panel)
{
	...
	if (sel == 0) {
		lcd_top[0]->tcon_clk_src.bits.lcd0_clk_src = panel->lcd_dsi_clk_if;
		lcd_top[0]->tcon_clk_src.bits.combo_phy0_clk_src = panel->lcd_dsi_clk_if;
		if (panel->lcd_tcon_mode == DISP_TCON_DUAL_DSI)
			lcd_top[0]->tcon_clk_src.bits.combo_phy1_clk_src = panel->lcd_dsi_clk_if;
	} else if (sel == 1) {
		lcd_top[0]->tcon_clk_src.bits.lcd1_clk_src = panel->lcd_dsi_clk_if;
		lcd_top[0]->tcon_clk_src.bits.combo_phy1_clk_src = panel->lcd_dsi_clk_if;
	}
	return 0;
}
