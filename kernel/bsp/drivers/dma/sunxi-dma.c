...
#define DMA_IRQ_DSP_DISABLE_MASK 	0xFF00
...
/*
 * The dma IP V3.2, like sun55iw3 dsp dma etc., uses the number of dma channels from the
 * device tree node.
 */
...
static int sun6i_dma_probe(struct platform_device *pdev)
{
	...
	if (sdc->cfg->has_dsp_mbus_clk) {
		sdc->clk_dsp_mbus = devm_clk_get(&pdev->dev, "dsp-mbus");
		if (IS_ERR(sdc->clk_dsp_mbus)) {
			dev_err(&pdev->dev, "No dsp mbus clock specified\n");
			return PTR_ERR(sdc->clk_dsp_mbus);
		}
	}
	...
	if (sdc->cfg->has_dsp_mbus_clk) {
		ret = clk_prepare_enable(sdc->clk_dsp_mbus);
		if (ret) {
			dev_err(&pdev->dev, "Couldn't enable dsp mbus clock\n");
	...
	if (sdc->cfg->has_multicore_shared) {
		...
		writel(DMA_IRQ_DSP_DISABLE_MASK, sdc->base + DMA_IRQ_DSP_EN_REG);
	}
...
err_dsp_mbus_clk_disable:
	if (sdc->cfg->has_dsp_mbus_clk)
		clk_disable_unprepare(sdc->clk_dsp_mbus);
