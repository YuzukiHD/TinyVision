...
extern char country_abbr[4];
#ifdef CONFIG_AUTO_CUSTREG
extern bool country_set;
extern int cur_cnt;
#endif
										   ...
										   struct ipc_e2a_msg *msg)
{
	...
	if (chan != NULL) {
		...
		if(!country_set && cur_cnt < MAX_SEARCH_CNT) {
			country_id = cfg80211_find_ie(WLAN_EID_COUNTRY, mgmt->u.beacon.variable,
								ind->length - offsetof(struct ieee80211_mgmt, u.beacon.variable));
			if (country_id) {
				strncpy(country_abbr, country_id + 2, 2);
				fdflag = true;
			}
			cur_cnt += 1;
			printk("find cnt: %d\n", cur_cnt);
		}else if (!country_set && cur_cnt >= MAX_SEARCH_CNT) {
			fdflag = true;
		}
		...
		if (fdflag) {
			...
			regulatory_set_wiphy_regd_sync(rwnx_hw->wiphy, getRegdomainFromRwnxDB(rwnx_hw->wiphy, country_abbr));
#else
			regulatory_set_wiphy_regd_sync_rtnl(rwnx_hw->wiphy, getRegdomainFromRwnxDB(rwnx_hw->wiphy, country_abbr));
#endif
			rtnl_unlock();
#else
			wiphy_apply_custom_regulatory(rwnx_hw->wiphy, getRegdomainFromRwnxDB(rwnx_hw->wiphy, country_abbr));
