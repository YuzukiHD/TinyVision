...
static void show_vma_header_prefix(struct seq_file *m, struct vm_area_struct *vma,
...
show_map_vma(struct seq_file *m, struct vm_area_struct *vma, int is_pid)
{
	...
	show_vma_header_prefix(m, vma, start, end, flags, pgoff, dev, ino);
...
static int show_smap(struct seq_file *m, void *v, int is_pid)
{
	...
	if (!rollup_mode) {
		...
		show_vma_header_prefix(
			m, vma, mss->first_vma_start, vma->vm_end, 0, 0, 0, 0);
		seq_pad(m, ' ');
		seq_puts(m, "[rollup]\n");
	} else {
		ret = SEQ_SKIP;
