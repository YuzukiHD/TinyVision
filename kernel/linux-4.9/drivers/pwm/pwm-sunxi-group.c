...
//		sunxi_pwm_writel(chip, group_reg_offset, value);
	...
	if (pc->ephy_channel > 0) {
		/*
		 * Init pwm channel as ephy clk
		 * Open pwm channel bit bypass and bit gate
		 */
		value = sunxi_pwm_readl(chip, PCGR);
		value |= BIT(pc->ephy_channel) << 16 |
			BIT(pc->ephy_channel);
				...
				struct pwm_state *state)
{
	...
	sel = pwm->pwm - chip->base;
	reg_offset = PWM_PCR_BASE + sel * PWM_PCR_REG_OFFSET;
...
static int sunxi_pwm_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(np, "ephy-channel", &pwm->ephy_channel);
	if (ret < 0) {
