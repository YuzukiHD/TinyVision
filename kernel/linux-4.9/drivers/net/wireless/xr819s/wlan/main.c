...
static void xradio_set_mac_addrs(struct xradio_common *hw_priv)
{
	...
	/*3.Get MAC address saved in file. */
#ifdef XRADIO_MACPARAM_HEX
...
#endif
	if (ret >= 0 && MACADDR_VAILID(macaddr)) {
		xradio_dbg(XRADIO_DBG_NIY, "Use File MAC address!\n");
		goto exit;
	}


	/*4.Use random MAC address and try to save it in file. */
...
#endif
		if (ret < 0)
			xradio_dbg(XRADIO_DBG_WARN, "Failed to save MAC addr in file:%s!\n",
				   WIFI_CONF_PATH);
	} else {
