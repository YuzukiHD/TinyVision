...
#define REG_TWO_MAC_ENABLE 0x0103
#define TWO_MAC_ENABLE_VALUE 0x0
...
#endif /* defined(CONFIG_MAC_LOOPBACK_DRIVER) || defined(CONFIG_TWO_MAC_DRIVER) */
...
thread_return lbk_thread(thread_context context)
{
	...
	do {
		...
		dbg_dump_pkt("RX packet", ploopback->rxbuf, ploopback->rxsize);
		...
		if ((ploopback->bstop == _TRUE) ||
			...
			sprintf(ploopback->msg, "loopback result: ok=%d%%(%d/%d),error=%d%%(%d/%d)\n", \
...
static struct xmit_frame *create_twomac_pkt(PADAPTER padapter, u8 *dst_mac, u32 size)
{
	struct xmit_priv *pxmitpriv;
	struct xmit_frame *pframe;
	struct xmit_buf *pxmitbuf;
	...
	_rtw_memcpy(pattrib->src, adapter_mac_addr(padapter), ETH_ALEN);
	_rtw_memcpy(pattrib->ta, pattrib->src, ETH_ALEN);
	_rtw_memcpy(pattrib->dst, dst_mac, ETH_ALEN);
	_rtw_memcpy(pattrib->ra, pattrib->dst, ETH_ALEN);
...
void two_mac_test(PADAPTER padapter, u8 *mac_addr, u32 size)
{
	struct xmit_priv *pxmitpriv;
	struct xmit_buf *pxmitbuf;
	struct xmit_frame *pxmitframe;
	...
	err = initpseudoadhoc(padapter);
	if (err == _FAIL) {
		RTW_INFO("loopback FAIL! 1.1 init ad-hoc FAIL!");
		return;
	}

	err = createpseudoadhoc(padapter);
	if (err == _FAIL) {
		RTW_INFO("loopback FAIL! 1.2 create ad-hoc master FAIL!");
		return;
	}
	...
	pxmitframe = create_twomac_pkt(padapter, mac_addr, pktsize);
	if (pxmitframe == NULL)
		RTW_INFO("loopback FAIL! 3. create Packet FAIL!");

	txsize = TXDESC_SIZE + pxmitframe->attrib.last_txcmdsz;
	ff_hwaddr = rtw_get_ff_hwaddr(pxmitframe);

	RTW_INFO("%s: wirte port tx_size=%d\n", __func__, txsize);

	rtw_write_port(padapter, ff_hwaddr, txsize, (u8 *)pxmitframe->pxmitbuf);
	...
	union iwreq_data *wrqu, char *extra)
{
	...
	if (strcmp(pch, "2mac_init") == 0) {
		...
		rtw_write8(padapter, REG_TWO_MAC_ENABLE, TWO_MAC_ENABLE_VALUE);
		RTW_INFO("Write 0x%03x to 0x%02x, enable 2MAC\n",
			REG_TWO_MAC_ENABLE, TWO_MAC_ENABLE_VALUE);

	} else if (strcmp(pch, "2mac_test") == 0) {
		u8 num;
		u32 size;
		...
		sscanf(pch, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &mac_addr[0], &mac_addr[1], &mac_addr[2]
		...
		RTW_INFO("%s: 2MAC test, ta="MAC_FMT", size=%u\n",
				    __func__, MAC_ARG(mac_addr), size);

		two_mac_test(padapter, mac_addr, size);
