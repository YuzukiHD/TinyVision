...
#if (HALMAC_8733B_SUPPORT && HALMAC_SDIO_SUPPORT)

#define WLAN_ACQ_NUM_MAX	8

static enum halmac_ret_status
chk_oqt_8733b(struct halmac_adapter *adapter, u32 tx_agg_num, u8 *buf,
	      u8 macid_cnt);

static enum halmac_ret_status
update_oqt_free_space_8733b(struct halmac_adapter *adapter);

static enum halmac_ret_status
update_sdio_free_page_8733b(struct halmac_adapter *adapter);

static enum halmac_ret_status
update_ac_empty_8733b(struct halmac_adapter *adapter, u8 value);

static enum halmac_ret_status
chk_qsel_8733b(struct halmac_adapter *adapter, u8 qsel_first, u8 *pkt,
	       u8 *macid_cnt);

static enum halmac_ret_status
chk_dma_mapping_8733b(struct halmac_adapter *adapter, u16 **cur_fs,
		      u8 qsel_first);

static enum halmac_ret_status
chk_rqd_page_num_8733b(struct halmac_adapter *adapter, u8 *buf, u32 *rqd_pg_num,
		       u16 **cur_fs, u8 *macid_cnt, u32 tx_agg_num);

/**
 * init_sdio_cfg_8733b() - init SDIO
...
init_sdio_cfg_8733b(struct halmac_adapter *adapter)
{
	u32 value32;
...
/**
 * mac_pwr_switch_sdio_8733b() - switch mac power
 * @adapter : the adapter of halmac
 * @pwr : power state
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 ...
 */
enum halmac_ret_status
mac_pwr_switch_sdio_8733b(struct halmac_adapter *adapter,
			  enum halmac_mac_power pwr)
{
	...
	PLTFM_MSG_TRACE("[TRACE]%s ===>\n", __func__);
	PLTFM_MSG_TRACE("[TRACE]8733B pwr seq ver = %s\n",
			HALMAC_8733B_PWR_SEQ_VER);
...
/**
 * halmac_tx_allowed_sdio_87xx() - check tx status
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
tx_allowed_sdio_8733b(struct halmac_adapter *adapter, u8 *buf, u32 size)
{
	u16 *cur_fs = NULL;
	u32 cnt;
	u32 tx_agg_num;
	...
	status = chk_rqd_page_num_8733b(adapter, buf, &rqd_pg_num, &cur_fs,
	...
	do {
		if ((u32)(*cur_fs + fs_info->pubq_pg_num) > rqd_pg_num) {
			status = chk_oqt_8733b(adapter, tx_agg_num, buf,
					       macid_cnt);
			if (status != HALMAC_RET_SUCCESS) {
		...
		update_sdio_free_page_8733b(adapter);
...
/**
 * cfg_tx_fmt_sdio_8733b() - config sdio tx address format
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
cfg_tx_fmt_sdio_8733b(struct halmac_adapter *adapter,
...
/**
 * halmac_reg_read_8_sdio_87xx() - read 1byte register
 * @adapter : the adapter of halmac
 ...
 */
u8
reg_r8_sdio_8733b(struct halmac_adapter *adapter, u32 offset)
{
	u8 value8;
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	...
	if ((offset & 0xFFFF0000) == 0) {
		if (adapter->pwr_off_flow_flag == 1 ||
			...
			value8 = (u8)r_indir_sdio_87xx(adapter, offset,
						       HALMAC_IO_BYTE);
		} else {
...
/**
 * halmac_reg_write_8_sdio_87xx() - write 1byte register
 * @adapter : the adapter of halmac
 * @offset : register offset
 ...
 */
enum halmac_ret_status
reg_w8_sdio_8733b(struct halmac_adapter *adapter, u32 offset, u8 value)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	enum halmac_sdio_cmd53_4byte_mode cmd53_4byte =
						adapter->sdio_cmd53_4byte;
	...
	if ((adapter->pwr_off_flow_flag == 1 ||
	     ...
	     cmd53_4byte == HALMAC_SDIO_CMD53_4BYTE_MODE_R) &&
	    (offset & 0xFFFF0000) == 0) {
		w_indir_sdio_87xx(adapter, offset, value, HALMAC_IO_BYTE);
	} else {
		if ((offset & 0xFFFF0000) == 0)
			offset |= WLAN_IOREG_OFFSET;
		status = cnv_to_sdio_bus_offset_87xx(adapter, &offset);
...
/**
 * halmac_reg_read_16_sdio_87xx() - read 2byte register
 * @adapter : the adapter of halmac
 * @offset : register offset
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 ...
 */
u16
reg_r16_sdio_8733b(struct halmac_adapter *adapter, u32 offset)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	union {
		__le16 word;
	...
	if ((offset & 0xFFFF0000) == 0 &&
	    adapter->halmac_state.mac_pwr == HALMAC_MAC_POWER_OFF) {
		value16.byte[0] = (u8)r_indir_sdio_87xx(adapter, offset,
							HALMAC_IO_BYTE);
		value16.byte[1] = (u8)r_indir_sdio_87xx(adapter, offset + 1,
							HALMAC_IO_BYTE);
...
/**
 * halmac_reg_write_16_sdio_87xx() - write 2byte register
 * @adapter : the adapter of halmac
 * @offset : register offset
 * @value : register value
 * Author : KaiYuan Chang/Ivan Lin
 ...
 */
enum halmac_ret_status
reg_w16_sdio_8733b(struct halmac_adapter *adapter, u32 offset, u16 value)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	...
	if (adapter->halmac_state.mac_pwr == HALMAC_MAC_POWER_OFF ||
		...
		if ((offset & 0xFFFF0000) == 0 && ((offset & (2 - 1)) == 0)) {
			status = w_indir_sdio_87xx(adapter, offset, value,
						   HALMAC_IO_WORD);
		} else {
			if ((offset & 0xFFFF0000) == 0)
				offset |= WLAN_IOREG_OFFSET;
...
/**
 * halmac_reg_read_32_sdio_87xx() - read 4byte register
 * @adapter : the adapter of halmac
 * @offset : register offset
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 ...
 */
u32
reg_r32_sdio_8733b(struct halmac_adapter *adapter, u32 offset)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	union {
		__le32 dword;
	...
	if (((offset & 0xFFFF0000) == 0) &&
	    adapter->halmac_state.mac_pwr == HALMAC_MAC_POWER_OFF) {
		value32.byte[0] = (u8)r_indir_sdio_87xx(adapter, offset,
							HALMAC_IO_BYTE);
		value32.byte[1] = (u8)r_indir_sdio_87xx(adapter, offset + 1,
							HALMAC_IO_BYTE);
		value32.byte[2] = (u8)r_indir_sdio_87xx(adapter, offset + 2,
							HALMAC_IO_BYTE);
		value32.byte[3] = (u8)r_indir_sdio_87xx(adapter, offset + 3,
							HALMAC_IO_BYTE);
...
/**
 * halmac_reg_write_32_sdio_87xx() - write 4byte register
 * @adapter : the adapter of halmac
 * @offset : register offset
 * @value : register value
 * Author : KaiYuan Chang/Ivan Lin
 ...
 */
enum halmac_ret_status
reg_w32_sdio_8733b(struct halmac_adapter *adapter, u32 offset, u32 value)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	      ...
	      u8 macid_cnt)
{
	...
	case HALMAC_QSEL_BK_V2:
		...
		do {
			...
			update_oqt_free_space_8733b(adapter);
	...
	case HALMAC_QSEL_HIGH:
		if (tx_agg_num > OQT_ENTRY_NOAC_8733B)
...
update_oqt_free_space_8733b(struct halmac_adapter *adapter)
{
	...
	update_ac_empty_8733b(adapter, data[4]);
...
update_sdio_free_page_8733b(struct halmac_adapter *adapter)
{
	...
	update_ac_empty_8733b(adapter, data[12]);
...
update_ac_empty_8733b(struct halmac_adapter *adapter, u8 value)
{
	...
	if (free_space->ac_oqt_num == OQT_ENTRY_AC_8733B) {
		while (value > 0) {
			value = value & (value - 1);
			free_space->ac_empty++;
		};
		...
		PLTFM_MSG_TRACE("[TRACE]free_space->ac_oqt_num %d != %d\n",
				free_space->ac_oqt_num, OQT_ENTRY_AC_8733B);
	}
 ...
 */
enum halmac_ret_status
phy_cfg_sdio_8733b(struct halmac_adapter *adapter,
 ...
 */
enum halmac_ret_status
pcie_switch_sdio_8733b(struct halmac_adapter *adapter,
...
/**
 * intf_tun_sdio_8733b() - sdio interface fine tuning
 * @adapter : the adapter of halmac
 * Author : Ivan
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
intf_tun_sdio_8733b(struct halmac_adapter *adapter)
{
	return HALMAC_RET_SUCCESS;
}

/**
 * halmac_get_sdio_tx_addr_sdio_87xx() - get CMD53 addr for the TX packet
 * @adapter : the adapter of halmac
 * @buf : tx packet, include txdesc
 * @size : tx packet size
 ...
 */
enum halmac_ret_status
get_sdio_tx_addr_8733b(struct halmac_adapter *adapter, u8 *buf, u32 size,
		       u32 *cmd53_addr)
{
	       ...
	       u8 *macid_cnt)
{
	...
	if (qsel_first == qsel_now) {
		...
		if (*(fs_info->macid_map + macid + MACID_MAX_8733B) == 0) {
			*(fs_info->macid_map + macid + MACID_MAX_8733B) = 1;
			(*macid_cnt)++;
		}
	}
		       ...
		       u16 **cur_fs, u8 *macid_cnt, u32 tx_agg_num)
{
	...
	status = chk_dma_mapping_8733b(adapter, cur_fs, qsel_first);
	if (status != HALMAC_RET_SUCCESS)
		return status;

	for (i = 0; i < tx_agg_num; i++) {
		/*QSEL parser*/
		status = chk_qsel_8733b(adapter, qsel_first, pkt, macid_cnt);
		if (status != HALMAC_RET_SUCCESS)
			return status;

		/*Page number parser*/
		pkt_size = GET_TX_DESC_TXPKTSIZE(pkt) + GET_TX_DESC_OFFSET(pkt);
		*rqd_pg_num += (pkt_size >> TX_PAGE_SIZE_SHIFT_87XX) +
				((pkt_size & (TX_PAGE_SIZE_87XX - 1)) ? 1 : 0);
...
u32
get_sdio_int_lat_8733b(struct halmac_adapter *adapter)
{
...
enum halmac_ret_status
get_sdio_clk_cnt_8733b(struct halmac_adapter *adapter, u32 *cnt)
{
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	u8 value8;
...
enum halmac_ret_status
set_sdio_wt_en_8733b(struct halmac_adapter *adapter, u8 enable)
{
	u32 reg = REG_SDIO_MONITOR_2 + 2;
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;

	status = cnv_to_sdio_bus_offset_87xx(adapter, &reg);
	if (status != HALMAC_RET_SUCCESS)
		return status;
...
enum halmac_ret_status
set_sdio_clk_mon_8733b(struct halmac_adapter *adapter,
		       enum halmac_sdio_clk_monitor monitor)
{
	...
	set_sdio_wt_en_8733b(adapter, 0);
