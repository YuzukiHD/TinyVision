...
rtw_btc_set_gnt_bt(struct btc_coexist *btc, u8 state)
{
	if (btc->chip_para->lte_indirect_access) {
		btc->btc_write_linderct(btc, REG_LTE_IDR_COEX_CTRL,
							0xc000, state);
		btc->btc_write_linderct(btc, REG_LTE_IDR_COEX_CTRL,
							0x0c00, state);
	} else {
			  ...
			  u8 phase)
{
	...
	case BTC_ANT_WOFF:
		...
		/*TODO: 8733b set 0x73[2] = 0 will set GNT_BT = 0, need modify BT fw*/
	...
	case BTC_ANT_5G:
		...
		/*8733b cannot set GNT_BT PTA control when wifi control by SW
		    GNT_WL should not reference GNT_BT when Antdiv on*/
		if (strcmp(btc->chip_para->chip_name, "8733b") == 0)
			rtw_btc_set_gnt_bt(btc, BTC_GNT_SW_HIGH);
		else
...
static void rtw_btc_action_bt_idle(struct btc_coexist *btc)
{
	...
	rtw_btc_set_rf_para(btc, NM_EXCU, btc->chip_para->wl_rf_para_rx[2]);
...
void rtw_btc_ex_display_coex_info(struct btc_coexist *btc)
{
	...
	if (!coex_sta->bt_disabled &&
		...
		//if (coex_dm->bt_status != BTC_BTSTATUS_NCON_IDLE) {
			...
			val = btc->btc_get_bt_reg(btc, 0, 0x9);
			coex_sta->bt_reg_rf_9 = (u16)val;
		//}
	}
	...
	if (is_bt_reply) {
		...
		if (coex_sta->bt_reg_vendor_ac == 0xffff) {
			//btc->btc_set_bt_reg(btc, 3, 0xdac, 0x10e000);
			val = btc->btc_get_bt_reg(btc, 3, 0xdac);
			coex_sta->bt_reg_vendor_ac = val;
		}
#if 0
	...
	if (coex_sta->bt_reg_vendor_ae == 0xffff ||
		...
		CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE,
			   ...
			   ((chip_para->scbd_bit_num == BTC_SCBD_32_BIT) ?
				btc->btc_read_scbd_32bit(btc, &scbd_32bit) :
				btc->btc_read_scbd(btc, &scbd)));
	else
		CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE,
...
#if 1
	if (coex_sta->bt_reg_vendor_ac == 0xffffffff)
		CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE,
			   "\r\n %-35s = x/ 0x%x",
			   "/0xac/ScBd(B->W)",
			   btc->btc_read_scbd(btc, &scbd));
