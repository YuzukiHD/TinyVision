...
static int esp_bt_send_frame(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if ((unsigned int)skb->data & 3) {
		printk(KERN_DEBUG "AW warming: data address(0x:%x) is not aligned with 4 bytes\n", skb->data);
		address_offset = ((unsigned int)skb->data % 4);
		remain = skb_headroom(skb) - sizeof(struct esp_payload_header) - address_offset;
		printk(KERN_DEBUG "AW warming: data offset(%d)\n", address_offset);
		if (remain >= 0) {
			skb_push(skb, sizeof(struct esp_payload_header) + address_offset);
			printk(KERN_DEBUG "AW info: data address offset %d bytes\n", address_offset);
		} else {
			printk(KERN_ERR "AW err: Not enough space for offset(remain:%d)\n", skb_headroom(skb));
			return -1;
		}
	} else {
	...
	if (address_offset > 0) {
		hdr->offset = cpu_to_le16(sizeof(struct esp_payload_header) + address_offset);
	} else {
		hdr->offset = cpu_to_le16(sizeof(struct esp_payload_header));
	}
	hdr->hci_pkt_type = hci_skb_pkt_type(skb);

	/* printk (KERN_DEBUG "H -> S: %d %d %d %d", len, hdr->offset,
		hdr->len, hdr->reserved1); */
