...
static void process_rx_packet(struct sk_buff *skb)
{
	...
	if (payload_header->if_type == ESP_SERIAL_IF) {
		...
		printk(KERN_INFO "ESP read for len %d\n", len);
		do {
			printk(KERN_INFO "ESP main b4  esp_serial_data_received exp readlen %d\n", len-ret_len);
			ret = esp_serial_data_received(0, (skb->data + offset + ret_len), (len - ret_len));
			printk(KERN_INFO "ESP main Aft esp_serial_data_received ret_len %d\n", ret_len);
			ret_len += ret;
		} while (ret_len < len);
#else
