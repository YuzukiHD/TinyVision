...
static u32 sunxi_disabled_opa_sel(void __iomem *reg_base)
{
	u32 reg_val;

	reg_val = readl(reg_base + GP_OPA_EN_REG);
	reg_val &= ~(0xff << 16);

	writel(reg_val, reg_base + GP_OPA_EN_REG);

	return 0;
}

static u32 sunxi_enable_opa_ch_select(void __iomem *reg_base,
		enum gp_channel_id id)
{
	u32 reg_val;
...
static int sunxi_gpadc_hw_init(struct sunxi_gpadc *sunxi_gpadc)
{
	...
	sunxi_gpadc_sample_rate_set(sunxi_gpadc->reg_base, OSC_24MHZ,
			sunxi_gpadc->gpadc_sample_rate);
	if (SUN50IW11P1_REV_C == sunxi_get_soc_ver())
		sunxi_disabled_opa_sel(sunxi_gpadc->reg_base);
	for (i = 0; i < sunxi_gpadc->channel_num; i++) {
...
//			}
			if (SUN50IW11P1_REV_C == sunxi_get_soc_ver())
				sunxi_enable_opa_ch_select(sunxi_gpadc->reg_base, i);
		}
	}
