...
#include "virt-dma.h"
	...
	|| defined(CONFIG_ARCH_SUN50IW11P1_C) \
...
static int sunxi_probe(struct platform_device *pdev)
{
	...
	int irq;
	int ret, i;
#ifdef CONFIG_ARCH_SUN50IW11P1_C
	int irq2;
#endif
	...
	if (irq2 < 0) {
		ret = irq2;
		dev_err(&pdev->dev, "Get IRQ Index 2 error!!!\n");
		free_irq(irq, sunxi_dev);
		goto irq_err;
	}
	ret = request_irq(irq2, sunxi_dma_interrupt, IRQF_SHARED,
			dev_name(&pdev->dev), sunxi_dev);
	if (ret) {
		dev_err(&pdev->dev, "NO IRQ found!!!\n");
		free_irq(irq, sunxi_dev);
	...
	sunxi_periph_reset_assert(sunxi_dev->ahb_clk);
	sunxi_periph_reset_deassert(sunxi_dev->ahb_clk);
...
#ifdef CONFIG_ARCH_SUN50IW11P1_C
	free_irq(platform_get_irq(pdev, 2), sunxi_dev);
#endif
	dma_pool_destroy(sunxi_dev->lli_pool);
