...
void sunxi_set_cur_vol_work(struct work_struct *work)
{
	...
	if (of_find_property(g_udc_pdev->dev.of_node, "det_vbus_supply", NULL))
		psy = power_supply_get_by_phandle(g_udc_pdev->dev.of_node,
						     "det_vbus_supply");
...
static void sunxi_vbus_det_work(struct work_struct *work)
{
	...
	if (!psy || IS_ERR(psy)) {
		...
		for (;;) {
			if (power_supply_get_property(psy,
					POWER_SUPPLY_PROP_ONLINE, &temp) < 0)
				break;
			if (usb_connect) {
				power_supply_put(psy);
				return;
	...
	if (udc->driver && udc->driver->disconnect) {
		udc->driver->disconnect(&udc->gadget);
#if !defined(CONFIG_AW_AXP) && defined(CONFIG_POWER_SUPPLY) && !defined SUNXI_USB_FPGA
		/*
