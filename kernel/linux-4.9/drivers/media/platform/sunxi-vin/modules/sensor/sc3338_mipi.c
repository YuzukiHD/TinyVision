/*
 * A V4L2 driver for sc3338 Raw cameras.
 *
...
MODULE_AUTHOR("zhj");
MODULE_DESCRIPTION("A low-level driver for SC3338 sensors");
MODULE_LICENSE("GPL");
...
/*
 * The SC3338 i2c address
 */
#define I2C_ADDR 0x60

#define SENSOR_NUM 0x2
#define SENSOR_NAME "sc3338_mipi"
#define SENSOR_NAME_2 "sc3338_mipi_2"
...
//cleaned_0x01_FT_SC3338_MIPI_27Minput_506.25Mbps_2lane_10bit_2304x1296_20fps
	{0x0103, 0x01},
	{0x0100, 0x00},
	{0x36e9, 0x80},
	{0x37f9, 0x80},
...
static int sc3338_sensor_vts;
static int sc3338_fps_change_flag;
...
//	data_type read_high = 0, read_low = 0;
	...
	if (!sc3338_fps_change_flag) {
		shutter = exp_val >> 4;
		if (shutter > sc3338_sensor_vts - 12) {
			frame_length = shutter + 12;
		} else
			frame_length = sc3338_sensor_vts;
...
//	data_type read_high = 0, read_low = 0;
	int sc3338_sensor_target_vts = 0;

	sc3338_fps_change_flag = 1;
	sc3338_sensor_target_vts = wsize->pclk/fps->fps/wsize->hts;

	if (sc3338_sensor_target_vts <= wsize->vts) {// the max fps = 20fps
		sc3338_sensor_target_vts = wsize->vts;
	} else if (sc3338_sensor_target_vts >= (wsize->pclk/wsize->hts)) {// the min fps = 1fps
		sc3338_sensor_target_vts = (wsize->pclk/wsize->hts) - 8;
	}

	sc3338_sensor_vts = sc3338_sensor_target_vts;
	sensor_dbg("target_fps = %d, sc3338_sensor_target_vts = %d, 0x320e = 0x%x, 0x320f = 0x%x\n", fps->fps,
		sc3338_sensor_target_vts, sc3338_sensor_target_vts >> 8, sc3338_sensor_target_vts & 0xff);
	sensor_write(sd, 0x320f, (sc3338_sensor_target_vts & 0xff));
	sensor_write(sd, 0x320e, (sc3338_sensor_target_vts >> 8));
//	sensor_read(sd, 0x320e, &read_high);
//	sensor_read(sd, 0x320f, &read_low);
//	sensor_print("[get write_times] 0x41 = 0x%x, 0x42 = 0x%x\n", read_high, read_low);
	sc3338_fps_change_flag = 0;
