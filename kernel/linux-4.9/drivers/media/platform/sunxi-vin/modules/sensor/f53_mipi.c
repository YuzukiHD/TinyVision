...
MODULE_AUTHOR("lcs");
MODULE_DESCRIPTION("A low-level driver for f53 sensors");
MODULE_LICENSE("GPL");

#define MCLK              (24*1000*1000)
#define V4L2_IDENT_SENSOR  0x0842
...
#define ID_VAL_LOW		0x42
#define SENSOR_FRAME_RATE 30

#define DBG_INFO(format, args...) (printk("[f53 MIPI INFO] LINE:%04d-->%s:"format, __LINE__, __func__, ##args))
#define DBG_ERR(format, args...)  (printk("[f53 MIPI ERR] LINE:%04d-->%s:"format, __LINE__, __func__, ##args))
...

{
	...
	shutter = exp_val >> 4;
	if (shutter > f53_sensor_vts - 4)
		frame_length = shutter + 4;
	else
		frame_length = f53_sensor_vts;
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
	...
	printk("------------------------\r\n");
	printk("this is f53 sensor init\r\n");
	printk("------------------------\r\n");
	printk("**********************************\r\n");
	printk("***********sensor init************\r\n");
	printk("**********************************\r\n");
...
static int sensor_reg_init(struct sensor_info *info)
{
	...
	f53_sensor_vts = wsize->vts;
	sensor_dbg("f53_sensor_vts = %d\n", f53_sensor_vts);
