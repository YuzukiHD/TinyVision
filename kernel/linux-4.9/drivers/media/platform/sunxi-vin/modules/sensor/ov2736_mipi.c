...
#define V4L2_IDENT_SENSOR  0x2732
...
#define SENSOR_NAME "ov2736_mipi"
...
static struct regval_list sensor_1080p30fps_mipi_regs[] = {
	{0x0103, 0x01},
	{0x0305, 0x3c},
	{0x0307, 0x00},
	{0x0308, 0x03},
...
static int ov2736_mipi_sensor_vts;
static int sensor_s_exp(struct v4l2_subdev *sd, unsigned int exp_val)
{
	data_type explow, exphigh;
	struct sensor_info *info = to_state(sd);

	if (exp_val > ((ov2736_mipi_sensor_vts - 6) << 4))
		exp_val = (ov2736_mipi_sensor_vts - 6) << 4;
	if (exp_val < 16 * 4)
		exp_val = 16 * 4;
	...
	sensor_write(sd, 0x3502, explow);	/*coarse integration time*/
...
static int sensor_s_gain(struct v4l2_subdev *sd, int gain_val)
{
	...
	if (gain_val <= 248) { // 15.5 * 16 = 248
		ana_gain = gain_val;
		dig_gain = 0x0400;
	} else {
		ana_gain = 248;
		dig_gain = (gain_val / 248) << 10;
	}

	ana_gain = ana_gain << 3;

	sensor_write(sd, 0x3508, (unsigned char)((ana_gain >> 8) & 0xff));
	sensor_write(sd, 0x3509, (unsigned char)(ana_gain & 0xff));

	sensor_write(sd, 0x350a, (unsigned char)((dig_gain >> 8) & 0xff));
	sensor_write(sd, 0x350b, (unsigned char)(dig_gain & 0xff));
...
static struct sensor_win_size sensor_win_sizes[] = {
	{
		...
		.gain_min   = 1<<4,
		.gain_max   = 32<<4,
		.regs       = sensor_1080p30fps_mipi_regs,
		.regs_size  = ARRAY_SIZE(sensor_1080p30fps_mipi_regs),
		.set_size   = NULL,
	},
};
...
static int sensor_reg_init(struct sensor_info *info)
{
	...
	ov2736_mipi_sensor_vts = wsize->vts;
