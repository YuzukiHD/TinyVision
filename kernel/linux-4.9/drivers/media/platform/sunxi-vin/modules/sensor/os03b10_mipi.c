/*
 * A V4L2 driver for os03b10 Raw cameras.
 *
...
MODULE_AUTHOR("lcs");
MODULE_DESCRIPTION("A low-level driver for OS03B10 sensors");
MODULE_LICENSE("GPL");

#define MCLK              (27*1000*1000)
#define V4L2_IDENT_SENSOR 0x53034210
...
#define SENSOR_NAME "os03b10_mipi"
...
static struct regval_list sensor_2304x1296p30_regs[] = {
	{0xfd, 0x00},
	{0x20, 0x00},
	{0xfd, 0x00},
	{0x36, 0x01},
	...
	{0xbd, 0x00},
	/* vlow 1x 0.71 ,vlow 2x 1.18 */
	{0xd7, 0xbe},
	/* vlow 3x 1.18 ,vlow 4x 1.3 */
	{0xd8, 0xef},
	/* vbl */
	{0xe8, 0x09},
	{0xe9, 0x05},
	...
	{0x06, 0x07},
	{0x07, 0x78},
	/* pd for dac_clk enbale */
	{0x08, 0x36},
	/* pldo=avdd;vbl row sel up */
	{0x09, 0x28},
	{0x0a, 0x0c},
	/*  rst_num2 */
	{0x0b, 0x06},
	...
	{0xbc, 0x00},
	{0xfd, 0x03},
	/* bit7=0 psnc on,=1 psnc off  */
	{0x02, 0x00},
	{0x05, 0x18},
	{0xfd, 0x02},
	/* digital window enable */
	{0x5e, 0x22},
	/* dpc on */
	{0x34, 0x80},
	{0xfd, 0x01},
...
static int sensor_s_exp(struct v4l2_subdev *sd, unsigned int exp_val)
{
	...
	if (all_exp < 4) {
		all_exp = 4;
	} else if (all_exp > 0xffff) {
		all_exp = 0xffff;
	}
	sensor_write(sd, 0xfd, 0x01);
	sensor_write(sd, 0x03, (all_exp >> 8) & 0xff);
	sensor_write(sd, 0x04, all_exp & 0xff);
...
static int sensor_get_fmt_mbus_core(struct v4l2_subdev *sd, int *code)
{
	...
	*code = MEDIA_BUS_FMT_SBGGR10_1X10; /* os03b10 support change the rgb format by itself */
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	...
	if (rdval != 0x53) {
		sensor_err(" read 0x02 return 0x%x\n", rdval);
		return -ENODEV;
	}

	sensor_read(sd, 0x03, &rdval);
	if (rdval != 0x03) {
		sensor_err(" read 0x03 return 0x%x\n", rdval);
		return -ENODEV;
	}

	sensor_read(sd, 0x04, &rdval);
	if (rdval != 0x42) {
		sensor_err(" read 0x04 return 0x%x\n", rdval);
		return -ENODEV;
	}

	sensor_read(sd, 0x05, &rdval);
	if (rdval != 0x10) {
		sensor_err(" read 0x05 return 0x%x\n", rdval);
		return -ENODEV;
	}
...
static struct sensor_win_size sensor_win_sizes[] = {

	{
		...
		.gain_min   = 1 << 4,
		.gain_max   = 512 << 4,
		.regs       = sensor_2304x1296p30_regs,
		.regs_size  = ARRAY_SIZE(sensor_2304x1296p30_regs),
		.set_size   = NULL,
	},
};
...
static int sensor_reg_init(struct sensor_info *info)
{
	...
	os03b10_sensor_vts = wsize->vts;
	sensor_flip_status = 0x0;
	sensor_dbg("os03b10_sensor_vts = %d\n", os03b10_sensor_vts);
