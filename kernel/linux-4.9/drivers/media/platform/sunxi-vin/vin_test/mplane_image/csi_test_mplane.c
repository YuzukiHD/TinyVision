...
static int camera_init(int sel, int mode)
{
	...
	if (-1 == ioctl(fd, VIDIOC_S_INPUT, &inp)) {
		...
		CLEAR(ve_online_cfg);
		ve_online_cfg.ve_online_en = 0;
		ve_online_cfg.dma_buf_num = BK_TWO_BUFFER;
...
static int camera_fmt_set(int mode)
{
...
#if 0
	struct vipp_shrink_cfg vipp_cfg;
	vipp_cfg.left = 0;
	vipp_cfg.top = 0;
	if (-1 == ioctl(fd, VIDIOC_SET_VIPP_SHRINK, &vipp_cfg)) {
		printf("VIDIOC_SET_VIPP_SHRINK error!\n");
		return -1;
	}
#endif
#if 0
	...
	CLEAR(speeddn_cfg);
	speeddn_cfg.pix_num = MIPI_TWO_PIXEL;
	speeddn_cfg.tdm_speed_down_en = 1;
	...
	while (count-- >= 0) {
		for (;;) {
			...
			if (r == 0) {
				...
				s.r.left = k % 540;
				s.r.top = k % 540;
				s.r.width = 1920 - 2 * s.r.left;
				s.r.height = 1080 - 2 * s.r.top;
