...
static void __init sunxi_nmi_pad_control(struct device_node *node)
{
	...
	if (of_property_read_u32(node, "pad-control-v1", &v)) {
		if (of_property_read_u32(node, "pad-control-v2", &v)) {
			return;
		}

		if (of_property_read_u32(node, "pad-control-en", &en)) {
			pr_err("no found pad-control-en\n");
			return;
		}

		if (of_property_read_u32(node, "pad-control-offset", &offset)) {
			pr_err("no found pad-control-offset\n");
			return;
		}
	}
