...
void eink_free_processed_node(struct buf_manager *buf_mgr, int upd_master)
{
	u32 buf_size = 0;
	u32 width = 0, height = 0;
	width = buf_mgr->processing_img_node->img->size.width;
	height = buf_mgr->processing_img_node->img->size.height;

	EINK_INFO_MSG("[%s]:width = %d, height = %d \n", __func__, width, height);
	buf_size = width * height;

	if ((buf_mgr->processing_img_node == NULL) || (buf_mgr->processing_img_node->img == NULL)) {
		pr_warn("[%s]:process node is NULL or img is NULL\n", __func__);
		return;
	}

	EINK_INFO_MSG("prcessing node upd_master = %d, wincalc_fin = %d, mode_select_fin = %d, coll_flag = 0x%llx\n",
			upd_master,
...
int eink_get_temperature_thread(void *parg)
{
	...
	for (;;) {
		...
		if (temperature == -1) {
			EINK_DEBUG_MSG("%s: get temperature=%d fail from tps65185\n",
								__func__, temperature);
		} else {
			manager->panel_temperature = temperature;
			EINK_DEBUG_MSG("%s: get temperature=%d degree from tps65185\n",
						__func__, manager->panel_temperature);
		}
#else
		manager->panel_temperature = 28;
		EINK_DEBUG_MSG("%s: set default temperature to %d degree\n",
						__func__, manager->panel_temperature);
#endif
...
int detect_fresh_thread(void *parg)
{
	...
	for (;;) {
		...
		if (buf_mgr->processing_img_node && buf_mgr->processing_img_node->img) {
			...
			case 0:
				eink_free_processed_node(buf_mgr, 0);
				break;
			case 1:
				eink_free_processed_node(buf_mgr, 1);
				break;
			case -1:
...
static int eink_intterupt_proc(int irq, void *parg)
{
	...
	if (upd_pic_accept == 0x10) {
		...
		/* debug */
		if (eink_get_print_level() == 8) {
			do_gettimeofday(&eink_mgr->acept_timer);
	...
	if (upd_pic_fin == 0x100) {
		...
		/* debug */
		if (eink_get_print_level() == 8) {
			do_gettimeofday(&eink_mgr->upd_pic_timer);
	...
	if (ee_fin == 0x1000000) {
		...
		/* calc time for debug */
		if (eink_get_print_level() == 8) {
			do_gettimeofday(&eink_mgr->etimer);
	...
	if (pipe_fin == 0x1000) {
		...
		/* debug */
		if (eink_get_print_level() == 8) {
			do_gettimeofday(&eink_mgr->pipe_timer);
