...
enum disp_enhance_dirty_flags {
	...
	ENH_FCM_DIRTY        = 0x1 << 16, /* fcm dirty */
...
struct disp_lcd_panel_fun {
	...
	int (*panel_id_check)(unsigned int sel);
};
...
struct disp_enhance {
	...
	s32 (*set_window)(struct disp_enhance *enhance,
			  struct disp_rect *window);
	s32 (*set_fcm_enhance)(struct disp_enhance *enhance, u32 value);
	s32 (*set_fcm_mode)(struct disp_enhance *enhance, u32 value);

	s32 (*get_fcm_mode)(struct disp_enhance *enhance);
	s32 (*get_fcm_enhance)(struct disp_enhance *enhance);
