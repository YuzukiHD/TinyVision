...
static s32 disp_enhance_get_fcm_mode(struct disp_enhance *enhance)
{
	struct disp_enhance_private_data *enhancep =
	    disp_enhance_get_priv(enhance);
	...
	return enhancep->config.info.fcm_mode;
}

static s32 disp_enhance_set_fcm_mode(struct disp_enhance *enhance, u32 value)
{
	unsigned long flags;
	struct disp_enhance_private_data *enhancep =
	    disp_enhance_get_priv(enhance);
	...
	spin_lock_irqsave(&enhance_data_lock, flags);
	enhancep->config.info.fcm_mode = value;
	enhancep->config.flags |= ENH_FCM_DIRTY;
	spin_unlock_irqrestore(&enhance_data_lock, flags);
...
static s32 disp_enhance_get_fcm_enhance(struct disp_enhance *enhance)
{
	struct disp_enhance_private_data *enhancep =
	    disp_enhance_get_priv(enhance);
	...
	return enhancep->config.info.fcm_enhance;
}

static s32 disp_enhance_set_fcm_enhance(struct disp_enhance *enhance, u32 value)
{
	unsigned long flags;
	struct disp_enhance_private_data *enhancep =
	    disp_enhance_get_priv(enhance);
	...
	spin_lock_irqsave(&enhance_data_lock, flags);
	enhancep->config.info.fcm_enhance = value;
	enhancep->config.flags |= ENH_FCM_DIRTY;
	spin_unlock_irqrestore(&enhance_data_lock, flags);
...
s32 disp_init_enhance(struct disp_bsp_init_para *para)
{
	...
	for (disp = 0; disp < num_enhances; disp++) {
		...
		enhance->get_fcm_mode = disp_enhance_get_fcm_mode;
		enhance->set_fcm_mode = disp_enhance_set_fcm_mode;
		enhance->get_fcm_enhance = disp_enhance_get_fcm_enhance;
		enhance->set_fcm_enhance = disp_enhance_set_fcm_enhance;
