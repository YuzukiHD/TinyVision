	...
	if (panel->lcd_hv_if == LCD_HV_IF_CCIR656_2CYC) {
		if (panel->lcd_interlace) {
			lcd_dev[sel]->tcon0_basic0.bits.y =
			    panel->lcd_y / 2 - 1;
			if (panel->lcd_hv_syuv_fdly ==  LCD_HV_SRGB_FDLY_2LINE)
				lcd_dev[sel]->tcon0_basic2.bits.vt = 625;
			else if (panel->lcd_hv_syuv_fdly == LCD_HV_SRGB_FDLY_3LINE)
				lcd_dev[sel]->tcon0_basic2.bits.vt = 525;
			else
			...
			lcd_dev[sel]->tcon0_basic0.bits.y = panel->lcd_y - 1;
			if (panel->lcd_hv_syuv_fdly ==  LCD_HV_SRGB_FDLY_2LINE)
				lcd_dev[sel]->tcon0_basic2.bits.vt = 1250;
			else if (panel->lcd_hv_syuv_fdly == LCD_HV_SRGB_FDLY_3LINE)
				lcd_dev[sel]->tcon0_basic2.bits.vt = 1050;
			else
				lcd_dev[sel]->tcon0_basic2.bits.vt = panel->lcd_vt * 2;
		}
