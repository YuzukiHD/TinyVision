/* drivers/video/sunxi/disp2/disp/lcd/kd101na5.c
 *
...
static struct LCM_setting_table lcm_kd101na5_initialization_setting[] = {
		{ 0x11, 0, { } },    // SLPOUT
...
static void lcd_panel_init(u32 sel)
{
	...
	for (i = 0;; i++) {
		if (lcm_kd101na5_initialization_setting[i].count == REGFLAG_END_OF_TABLE)
			break;
		else if (lcm_kd101na5_initialization_setting[i].count == REGFLAG_DELAY) {
			sunxi_lcd_delay_ms(lcm_kd101na5_initialization_setting[i].para_list[0]);
		} else
			dsi_dcs_wr(sel, lcm_kd101na5_initialization_setting[i].cmd,
					lcm_kd101na5_initialization_setting[i].para_list,
					lcm_kd101na5_initialization_setting[i].count);
		/* break; */
	}
...
struct __lcd_panel kd101na5_panel = {
