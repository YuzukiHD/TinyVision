				...
				struct disp_enhance_config *config)
{
	...
	for (ch_id = 0; ch_id < chno; ch_id++) {
		...
		if (de_feat_is_support_fcm_by_chn(screen_id, ch_id)) {
			de_fcm_enable(screen_id, ch_id, config[0].info.enable);
			de_fcm_set_para(screen_id, config[0].info.fcm_enhance, config[0].info.fcm_mode);
			continue;
		}
...
int de_enhance_set_size(unsigned int screen_id, struct disp_rect *size)
{
	...
	for (ch_id = 0; ch_id < chno; ch_id++) {
		...
		if (de_feat_is_support_fcm_by_chn(screen_id, ch_id)) {
			de_fcm_set_size(screen_id, ch_id, size[ch_id].width,
				size[ch_id].height);
			de_fcm_set_window(screen_id, ch_id, demo_enable, tmp_win);
			continue;
		}
		/* fce */
...
int de_enhance_update_regs(unsigned int screen_id)
{
	...
	for (ch_id = 0; ch_id < chno; ch_id++) {
		if (de_feat_is_support_fcm_by_chn(screen_id, ch_id)) {
			de_fcm_update_regs(screen_id, ch_id);
			continue;
		}
