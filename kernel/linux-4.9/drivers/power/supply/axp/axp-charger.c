...
static int bmu1760_bat_det(struct axp_regmap *map)
{
	u8 val;
	axp_regmap_read(map, PMU_VBUS_GD, &val);
	if (!(val & 0x02))
		return 1;

	axp_regmap_read(map, PMU_OCV_DATA_H, &val);
	if (val & 0xff)
		return 1;

	axp_regmap_read(map, PMU_OCV_DATA_L, &val);
	if (val & 0xff)
		return 1;
	...
	if (batt->det_unused == 0) {
		if (batt->det_valid == 1) {
			...
			if (chg_dev->bat_det == 0) {
				if (chg_dev->bmu_version_d)
					chg_dev->bat_det = bmu1760_bat_det(map);
				else
