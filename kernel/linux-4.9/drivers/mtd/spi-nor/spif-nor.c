 ...
 * Copyright (C) 2022, Allwinner Technology Co., Ltd. <www.allwinnertech.com>
 *
 * For the SUNXI SPI Flash controller
 */
...
#define SUNXI_SPIF_NOR_MODULE_VERSION "1.0.1"
...
#define USE_ALL_DTR             BIT(19) /* flash supports ALL DTR mode */
#define USE_RX_DTR              BIT(20) /* flash supports RX DTR mode */
#define USE_IO_MODE		BIT(21) /* Address and data line width */
...
static void spi_dtr_select(struct spi_nor *nor, struct spi_mem_op *op)
{
	if (nor->info->flags & USE_ALL_DTR)
		op->dtr_status = 1;
	else if (nor->info->flags & USE_RX_DTR) {
		if (op->cmd.opcode == SPINOR_OP_READ_1_4_4_DTR ||
			op->cmd.opcode == SPINOR_OP_READ_1_4_4_DTR_4B)
			op->dtr_status = 1;
		else
			op->dtr_status = 0;
	} else
		dev_warn(nor->dev, "The DTR function is not configured\n");
}

static int spi_mem_exec_op(struct spi_nor *nor, struct spi_mem_op *op)
{
	...
	if (spi_nor_protocol_is_dtr(nor->read_proto) ||
			spi_nor_protocol_is_dtr(nor->write_proto))
		spi_dtr_select(nor, op);

	t.tx_buf = (const void *)op;
	t.len = sizeof(struct spi_mem_op);

	ret = sunxi_spif_controller_ops(spi, &t);
	if (ret)
		return ret;
...
static int spi_nor_write_one_data(struct spi_nor *nor, loff_t to, const u8 *buf)
{
	struct spi_mem_op op =
			SPI_MEM_OP(SPI_MEM_OP_CMD(nor->program_opcode, 1),
				   SPI_MEM_OP_ADDR(nor->addr_width, to, 1),
	...
	op.cmd.buswidth = spi_nor_get_protocol_inst_nbits(nor->write_proto);
	op.addr.buswidth = spi_nor_get_protocol_addr_nbits(nor->write_proto);
	op.mode.buswidth = spi_nor_get_protocol_data_nbits(nor->write_proto);
...
static u8 spi_nor_convert_3to4_read(u8 opcode)
{
	static const u8 spi_nor_3to4_read[][2] = {
		...
		{ SPINOR_OP_READ_1_1_8,	SPINOR_OP_READ4_1_1_8 },
		{ SPINOR_OP_READ_1_8_8,	SPINOR_OP_READ4_1_8_8 },
...
static int spi_nor_erase_sector(struct spi_nor *nor, u32 addr)
{
	struct spi_mem_op op =
		SPI_MEM_OP(SPI_MEM_OP_CMD(nor->erase_opcode, 1),
			   SPI_MEM_OP_ADDR(nor->addr_width, spi_nor_convert_addr(nor, addr), 1),
			   SPI_MEM_OP_NO_MODE,
			   SPI_MEM_OP_NO_DUMMY,
			   SPI_MEM_OP_NO_DATA);
 ...
 * erase_size keep 4KB.
 */
static int spif_nor_erase_4k(struct mtd_info *mtd, struct erase_info *instr)
{
	struct spi_nor *nor = mtd_to_spi_nor(mtd);
	u32 addr, len;
	uint32_t rem;
	...
	if (spi_nor_has_uniform_erase(nor)) {
		div_u64_rem(instr->len, erasesize, &rem);
		if (rem)
			return -EINVAL;
	}
...
static const struct flash_info spi_nor_ids[] = {
	...
	{
		"GD25F256FYIG", INFO(0xc84319, 0, 64 * 1024, 512,
			SPI_NOR_DUAL_READ | SPI_NOR_QUAD_READ |
	...
	/* PUYA */
	{ "p25q128h", INFO(0x852018, 0, 64 * 1024, 256, SPI_NOR_DUAL_READ | SPI_NOR_QUAD_READ | USE_IO_MODE | USE_RX_DTR) },
	{ "p25q128", INFO(0x856018, 0, 64 * 1024, 256, SECT_4K | SPI_NOR_DUAL_READ | SPI_NOR_QUAD_READ) },
	{ "p25q128", INFO(0x852018, 0, 64 * 1024, 256, SECT_4K | SPI_NOR_DUAL_READ | SPI_NOR_QUAD_READ) },
	...
	/* XTX */
	{ "xt25p128fw",  INFO(0x0b4018, 0, 64 * 1024, 256, SECT_4K | SPI_NOR_DUAL_READ | SPI_NOR_QUAD_READ |
				USE_IO_MODE | USE_RX_DTR) },
       /* ZB */
			...
			size_t *retlen, u_char *buf)
{
	...
	while (len) {
		...
		if (read_len % 4) {
			cache_len = (read_len + 4) & ~0x3;
			cache_buf = vmalloc(cache_len);
			ret = nor->read(nor, addr, cache_len, cache_buf);
			memcpy(buf, cache_buf, read_len);
			vfree(cache_buf);
		} else
...
{
	struct spi_nor *nor = mtd_to_spi_nor(mtd);
	size_t page_offset, page_remain, i, j;
	ssize_t ret;
	size_t remainder, align = 4;
	...
	for (i = 0; i < len; ) {
		...
		/* Compatibility aw1886 requires 4-byte alignment writes */
		remainder = page_remain % align;
		page_remain = page_remain & ~(align - 1);
		if (page_remain) {
			write_enable(nor);
			ret = nor->write(nor, addr, page_remain, buf_p);
			if (ret < 0)
				goto write_err;
			written += ret;
		...
		for (j = 0; j < remainder;) {
			write_enable(nor);
			ret = spi_nor_write_one_data(nor, addr + page_remain + j,
					buf_p + page_remain + j);
			if (ret < 0)
				goto write_err;
			written += ret;
...
static void spif_nor_restore(struct spi_nor *nor)
{
...
EXPORT_SYMBOL_GPL(spif_nor_restore);
				    ...
				    size_t count, loff_t *ppos)
{
	...
	if (!strncmp(spinordbg_priv.param, "freq", 4)) {
		...
		switch (spi_nor_get_protocol_data_nbits(nor->read_proto))	{
		case OCTAL_MODE:
			strcpy(spinordbg_priv.status, "OCTAL");
			break;
		case QUAD_MODE:
		...
		case DUAL_MODE:
			...
			break;
		case SINGLE_MODE:
			if (nor->read_opcode == SPINOR_OP_READ_FAST)
				strcpy(spinordbg_priv.status, "FAST");
			else if (nor->read_opcode == SPINOR_OP_READ)
...
static int spif_debug_init(void)
{
...
static int spif_security_regiser_is_locked(struct mtd_info *mtd, u8 sr_num)
{
	u8 val, mask;
...
static int spif_security_register_lock(struct mtd_info *mtd, u8 sr_num)
{
	u8 val, mask;
	int ret;
	struct spi_nor *nor = mtd_to_spi_nor(mtd);
...
static int spif_security_regiser_erase(struct mtd_info *mtd, loff_t addr)
{
	u8 program_opcode;
	u8 addr_width;
	int write_proto;
...
static int spif_security_regiser_read_data(struct mtd_info *mtd,
					loff_t addr, loff_t len, u_char *buf)
{
	...
	while (len) {
		loff_t addr_temp = addr;
		size_t read_len = len;

		addr_temp = spi_nor_convert_addr(nor, addr_temp);

		if (read_len % 4) {
			cache_len = (read_len + 4) & ~0x3;
			cache_buf = vmalloc(cache_len);
			ret = nor->read(nor, addr_temp, cache_len, cache_buf);
			memcpy(buf, cache_buf, read_len);
			vfree(cache_buf);
		} else
			ret = nor->read(nor, addr_temp, read_len, buf);
		if (ret == 0) {
			/* We shouldn't see 0-length reads */
			ret = -EIO;
			goto read_err;
...
static int spif_security_regiser_write_data(struct mtd_info *mtd,
					loff_t addr, loff_t len, u_char *buf)
{
	u8 program_opcode;
	int write_proto;
	...
	ret = spif_security_regiser_erase(mtd, addr);
	if (ret)
		goto write_err;
	...
	for (i = 0; i < len; ) {
		 ...
		 */
		if (hweight32(nor->page_size) == 1) {
			page_offset = addr_temp & (nor->page_size - 1);
		} else {
			uint64_t aux = addr_temp;
		...
		if (page_remain) {
			write_enable(nor);
			ret = nor->write(nor, addr_temp, page_remain, buf_p);
			if (ret < 0)
				goto write_err;
			written += ret;
		...
		for (j = 0; j < remainder;) {
			write_enable(nor);
			ret = spi_nor_write_one_data(nor, addr_temp + page_remain + j,
					buf_p + page_remain + j);
			if (ret < 0)
				goto write_err;
			written += ret;
...
int spif_nor_scan(struct spi_nor *nor, const char *name, enum read_mode mode)
{
	const struct flash_info *info;
	struct device *dev = nor->dev;
	struct mtd_info *mtd = &nor->mtd;
	 ...
	 * than 1KB) after spif_nor_scan() returns.
	 */
	...
	mtd->_erase_4k = spif_nor_erase_4k;
	mtd->_read = spi_nor_read;
	mtd->_resume = spi_nor_resume;
	mtd->_suspend = spi_nor_suspend;
#ifdef CONFIG_SPI_FLASH_SR
	mtd->_security_regiser_is_locked = spif_security_regiser_is_locked;
	mtd->_security_register_lock = spif_security_register_lock;
	mtd->_security_regiser_read_data = spif_security_regiser_read_data;
	mtd->_security_regiser_write_data = spif_security_regiser_write_data;
#endif
	...
	sunxi_spif_update_sample_delay_para(mtd, nor->spimem->spi);
...
EXPORT_SYMBOL_GPL(spif_nor_scan);

static int spif_nor_probe(struct spi_mem *spimem)
{
	...
	ret = spif_nor_scan(nor, flash_name, mode);
	if (ret)
		return ret;
...
static int spif_nor_remove(struct spi_mem *spimem)
{
...
static void spif_nor_shutdown(struct spi_mem *spimem)
{
	struct spi_nor *nor = spi_mem_get_drvdata(spimem);
 ...
 */
static const struct spi_device_id spif_nor_dev_ids[] = {
	/*
...
MODULE_DEVICE_TABLE(spi, spif_nor_dev_ids);

static const struct of_device_id spif_nor_of_table[] = {
	/*
	 * Generic compatibility for SPI NOR that can be identified by the
	 * JEDEC READ ID opcode (0x9F). Use this, if possible.
	 */
	{ .compatible = "jedec,spif-nor" },
	{ /* sentinel */ },
};
MODULE_DEVICE_TABLE(of, spif_nor_of_table);
 ...
 */
static struct spi_mem_driver spif_nor_driver = {
	.spidrv = {
		.driver = {
			.name = "spif-nor",
			.of_match_table = spif_nor_of_table,
		},
		.id_table = spif_nor_dev_ids,
	},
	.probe = spif_nor_probe,
	.remove = spif_nor_remove,
	.shutdown = spif_nor_shutdown,
};
module_spi_mem_driver(spif_nor_driver);


MODULE_LICENSE("GPL v2");
MODULE_AUTHOR("lujianliang <lujianliang@allwinnertech.com>");
MODULE_AUTHOR("Mike Lavender");
MODULE_VERSION(SUNXI_SPIF_NOR_MODULE_VERSION);
