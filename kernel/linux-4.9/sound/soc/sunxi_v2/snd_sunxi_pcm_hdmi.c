			      ...
			      snd_pcm_uframes_t count)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (sunxi_pcm.hdmi_fmt > HDMI_FMT_PCM) {
			char *hdmihw_area = sunxi_pcm.raw_dma_area
					    + 2 * frames_to_bytes(runtime, hwoff);
			snd_sunxi_transfer_format_61937_to_60958((int *)hdmihw_area,
								 (short *)hwbuf,
...
static struct snd_soc_platform_driver sunxi_soc_platform_raw = {
...
{
	SND_LOG_DEBUG(HLOG, "\n");
	return snd_soc_register_platform(dev, &sunxi_soc_platform_raw);
}
