/*
 * sound\soc\sunxi\sun50iw11\sunxi-sndcodec.c
 *
...
/*lpsd channel sel*/
static int sunxi_sndcodec_set_lpsd_chan(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct sunxi_sndcodec_priv *sndcodec_priv =
...
static int sunxi_sndcodec_get_lpsd_chan(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct sunxi_sndcodec_priv *sndcodec_priv =
...
static int sunxi_sndcodec_set_mad_standby_chan(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct sunxi_sndcodec_priv *sndcodec_priv =
...
static int sunxi_sndcodec_get_mad_standby_chan(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct sunxi_sndcodec_priv *sndcodec_priv =
...
static const struct snd_kcontrol_new sunxi_card_controls[] = {
	...
	SOC_ENUM_EXT("lpsd channel sel Function", lpsd_chan_sel_enum[0],
		sunxi_sndcodec_get_lpsd_chan, sunxi_sndcodec_set_lpsd_chan),
	SOC_ENUM_EXT("mad_standby channel sel Function",
		mad_standby_chan_sel_enum[0],
		sunxi_sndcodec_get_mad_standby_chan,
		sunxi_sndcodec_set_mad_standby_chan),
#endif
};
	...
	struct snd_pcm_hw_params *params)
{
	...
	ret = snd_soc_dai_set_sysclk(codec_dai, 0, freq, substream->stream);
	if (ret < 0) {
...
static int sunxi_card_dev_probe(struct platform_device *pdev)
{
	...
	struct sunxi_sndcodec_priv *sndcodec_priv;
	struct snd_soc_dai_link *dai_link = &sunxi_card_dai_link;
	int ret = 0;
