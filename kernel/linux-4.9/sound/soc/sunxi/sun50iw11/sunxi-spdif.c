...
#if IS_ENABLED(CONFIG_SND_SUNXI_SOC_SUN50IW11_SPDIF_RX_IEC61937)
						...
						unsigned int freq, int dir)
{
	...
	if (sunxi_spdif->active == 0) {
		...
		if (clk_set_rate(clk_info->clk_module_rx, 200*1000000)) {
			dev_err(sunxi_spdif->dev,
				"clk module_rx set rate to 200MHz failed\n");
			mutex_unlock(&sunxi_spdif->mutex);
			return -EBUSY;
		}
#endif
...
static int sunxi_spdif_suspend(struct snd_soc_dai *dai)
{
	...
	if (!IS_ERR_OR_NULL(clk_info->clk_periph0))
		clk_disable_unprepare(clk_info->clk_periph0);
#endif
...
#if IS_ENABLED(CONFIG_SND_SUNXI_SOC_SUN50IW11_SPDIF_RX_IEC61937)
	if (!IS_ERR_OR_NULL(clk_info->clk_periph0)) {
		ret = clk_prepare_enable(clk_info->clk_periph0);
		if (ret < 0) {
			dev_err(sunxi_spdif->dev, "periph0 clk prepare enable error:%d\n", ret);
			goto err_daudio_resume_clk_enable_periph0;
		}
	}
	if (!IS_ERR_OR_NULL(clk_info->clk_module_rx)) {
		ret = clk_prepare_enable(clk_info->clk_module_rx);
		if (ret < 0) {
			dev_err(sunxi_spdif->dev, "modlule_rx clk prepare enable error:%d\n", ret);
			goto err_daudio_resume_clk_enable_module_rx;
		}
	}
#endif
...
err_daudio_resume_pinstate:
	...
	if (!IS_ERR_OR_NULL(clk_info->clk_module_rx))
		clk_disable_unprepare(clk_info->clk_module_rx);
err_daudio_resume_clk_enable_module_rx:
	if (!IS_ERR_OR_NULL(clk_info->clk_periph0))
		clk_disable_unprepare(clk_info->clk_periph0);
err_daudio_resume_clk_enable_periph0:
#endif
	if (!IS_ERR_OR_NULL(clk_info->clk_module))
		clk_disable_unprepare(clk_info->clk_module);
...
#if IS_ENABLED(CONFIG_SND_SUNXI_SOC_SUN50IW11_SPDIF_RX_IEC61937)
	clk_info->clk_periph0 = of_clk_get(node, 5);
	if (IS_ERR_OR_NULL(clk_info->clk_periph0)) {
		dev_err(&pdev->dev, "Can't get pll_periph0 clk clocks!\n");
		ret = PTR_ERR(clk_info->clk_periph0);
		goto err_of_get_clk_periph0;
	}

	clk_info->clk_module_rx = of_clk_get(node, 6);
	if (IS_ERR(clk_info->clk_module_rx)) {
		dev_err(&pdev->dev, "Can't get spdifrx clocks!\n");
		ret = PTR_ERR(clk_info->clk_module_rx);
...
#if IS_ENABLED(CONFIG_SND_SUNXI_SOC_SUN50IW11_SPDIF_RX_IEC61937)
	if (clk_set_parent(clk_info->clk_module_rx, clk_info->clk_periph0)) {
		dev_err(&pdev->dev,
			"set parent of clk_module_rx to clk_periph0 failed!\n");
		ret = -EINVAL;
		goto err_clk_set_parent;
	}
#endif
...
#if IS_ENABLED(CONFIG_SND_SUNXI_SOC_SUN50IW11_SPDIF_RX_IEC61937)
	if (clk_prepare_enable(clk_info->clk_periph0)) {
		dev_err(&pdev->dev, "clk_periph0 enable failed\n");
		ret = -EBUSY;
		goto err_clk_enable_periph0;
	}
	if (clk_prepare_enable(clk_info->clk_module_rx)) {
		dev_err(&pdev->dev, "moduleclk enable failed\n");
		ret = -EBUSY;
		goto err_clk_enable_module_rx;
	}
#endif
...
err_devm_pinctrl_get:
	...
	if (!IS_ERR_OR_NULL(clk_info->clk_module_rx))
		clk_disable_unprepare(clk_info->clk_module_rx);
err_clk_enable_module_rx:
	if (!IS_ERR_OR_NULL(clk_info->clk_periph0))
		clk_disable_unprepare(clk_info->clk_periph0);
err_clk_enable_periph0:
#endif
	if (!IS_ERR_OR_NULL(clk_info->clk_module))
		clk_disable_unprepare(clk_info->clk_module);
...
err_of_get_clk_module_rx:
	if (!IS_ERR_OR_NULL(clk_info->clk_periph0))
		clk_put(clk_info->clk_periph0);
err_of_get_clk_periph0:
#endif
	if (!IS_ERR_OR_NULL(clk_info->clk_module))
