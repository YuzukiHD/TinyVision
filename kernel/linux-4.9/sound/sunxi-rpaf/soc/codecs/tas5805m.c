...
#define RETRIES              (3)
#define CONFIG_CRC_CHECKSUM  (0x69)

static void TAS5805m_switch_book(struct snd_soc_codec *codec,
	...
	snd_soc_write(codec, TAS5805M_REG_7F, book_id);
	//w 98 00 <page_id> switch to page |page_id|
	snd_soc_write(codec, TAS5805M_REG_00, page_id);
}
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	struct TAS5805m_priv *priv = snd_soc_codec_get_drvdata(codec);
...
static void tas5805m_set_volume(struct snd_soc_codec *codec, int vol)
{
	...
	TAS5805m_switch_book(codec, TAS5805M_BOOK_8C, TAS5805M_PAGE_2A);
	//w 58 24 xx xx xx xx
...
static int TAS5805m_get_volsw(struct snd_kcontrol *kcontrol,
	...
	mutex_lock(&priv->lock);
	TAS5805m_switch_book(codec, TAS5805M_BOOK_00, TAS5805M_PAGE_00);
...
static const struct snd_kcontrol_new TAS5805m_snd_controls[] = {
	...
	SOC_SINGLE_EXT_TLV("Digital Volume", TAS5805M_REG_4C, 0, 0xFF, 1,
...
static void transmit_registers(struct i2c_client *client, cfg_reg *r, int n)
{
	...
	while (i < n) {
		...
		case CFG_META_DELAY:
			...
			break;
		case CFG_META_BURST:
			i2c_write(client, (unsigned char *)&r[i+1], r[i].param);
			i +=  (r[i].param / 2) + 1;
			break;
		default:
			i2c_write(client, (unsigned char *)&r[i], 2);
			break;
		}
		i++;
	}
...
static void tas5805m_priv_init(struct TAS5805m_priv *priv)
{
	...
	for (i = 0; i < RETRIES; i++) {
		...
		transmit_registers(priv->client, registers,
