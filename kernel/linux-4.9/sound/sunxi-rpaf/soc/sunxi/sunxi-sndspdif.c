					...
					struct snd_pcm_hw_params *params)
{
	...
	case 96000:
	case 192000:
		clk_div = SUNXI_SPDIF_CLK_DIV0;
		clk_id = 0;
		freq = rate * 128 * clk_div;
		break;
	case 22050:
	case 44100:
		clk_div = SUNXI_SPDIF_CLK_DIV1;
		clk_id = 1;
		freq = rate * 128 * clk_div;
		break;
	case 176400:
		/*
		 * clk_src is 90316800, so the maximum of clk_div is 4
		 * there is still channel exchange with this clk_div.
		 */
		clk_div = 4;
		clk_id = 1;
		freq = rate * 128 * clk_div;
	...
	ret = snd_soc_dai_set_sysclk(cpu_dai, clk_id, freq, 0);
	if (ret < 0) {
