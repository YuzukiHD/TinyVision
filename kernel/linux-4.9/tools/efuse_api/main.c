...
int main(int argc, char **argv)
{
	...
	if (!strncmp(argv[1], "-er", 3)) {
		if (argv[4]) {
			if (sunxi_efuse_read(argv[2], simple_strtoul(argv[3], NULL, 16), simple_strtoul(argv[4], NULL, 16), buf) < 0) {
				printf("read efuse data fail\n");
				goto error;
			}
			printf("api buf:\n");
			for (ret = 0; ret < simple_strtoul(argv[4], NULL, 16); ret++)
				printf("%x ", buf[ret]);
			printf("\n");
		}
	} else if (!strncmp(argv[1], "-ew", 3)) {
		if (argv[4]) {
			if (sunxi_efuse_write(argv[2], simple_strtoul(argv[3], NULL, 16), argv[4]) < 0) {
