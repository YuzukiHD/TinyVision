...
__s32 spinand_super_page_write(__u32 nDieNum, __u32 nBlkNum, __u32 nPage, __u64 SectBitmap, void *pBuf, void *pSpare)
{
	...
	if (SectBitmap != FULL_BITMAP_OF_SUPER_PAGE) {
		PHY_ERR("%s: Fatal err -2, Wrong input parameter, unaligned write page SectBitmap: 0x%llx(%llu)-- 0x%llx\n",
				__func__, SectBitmap,  SectBitmap, FULL_BITMAP_OF_SUPER_PAGE);
		return -1;
	}
...
int spinand_physic_bad_block_check(unsigned int chip, unsigned int block)
{
	...
	if (spare[0] != 0xff) {
		PHY_ERR("bad block check block fail: %d %d %d ", chip, block, para.page);
