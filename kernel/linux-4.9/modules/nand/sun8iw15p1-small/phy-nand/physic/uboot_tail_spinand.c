...
int spinand_write_uboot_one(unsigned char *buf, unsigned int len, struct _boot_info *info_buf, unsigned int info_len, unsigned int counter)
{
	...
	if (real_len <= SPINAND_GetPhyblksize()) {
		ret = spinand_write_uboot_one_in_block(buf, len, phyinfo_buf, PHY_INFO_SIZE, counter);
	} else {
		ret = spinand_write_uboot_one_in_many_block(buf, len, phyinfo_buf, PHY_INFO_SIZE, counter);
	}
	return ret;
}
...
int spinand_dragonborad_test_one(unsigned char *buf, unsigned char *oob, unsigned int blk_num)
{
	...
	for (j = 0; j < pages_per_block; j++) {
		...
		if (spinand_physic_write_page(para.chip, para.block, para.page, para.sectorbitmap,
					para.mainbuf, para.oobbuf) != 0) {
			PHY_ERR("Warning. Fail in writing page %d in block %d.\n", para.page, para.block);
		}
	}
	for (j = 0; j < pages_per_block; j++) {
		...
		if (spinand_physic_read_page(para.chip, para.block, para.page, para.sectorbitmap,
					para.mainbuf, para.oobbuf) != 0) {
			PHY_ERR("Warning. Fail in read page %d in block %d.\n", j, para.block);
			return -1;
		}
