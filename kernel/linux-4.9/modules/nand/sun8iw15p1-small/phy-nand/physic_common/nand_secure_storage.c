...
int nand_secure_storage_write_init(unsigned int block)
{
	...
	if (nand_physic_erase_block(0, block) == 0) {
		for (i = 0; i < page_cnt_per_blk; i++) {
			nand_physic_write_page(0, block, i, page_size / 512, mbuf, spare);
		}
		mbuf[0] = 0X11;
...
int nand_secure_storage_read_one(unsigned int block, int item, unsigned char *mbuf, unsigned int len)
{
	...
	for (; item < page_cnt_per_blk; item += MAX_SECURE_STORAGE_ITEM) {
		MEMSET(spare, 0, 64);
		nand_physic_read_page(0, block, item, page_size / 512, mbuf, spare);
...
int nand_secure_storage_read(int item, unsigned char *buf, unsigned int len)
{
	...
	for (i = 0; i < pages_cnt_per_item; i++) {
		...
		ret = nand_secure_storage_read_one(nand_secure_storage_block, item, mbuf, len);
		if (ret != 0) {
			ret = nand_secure_storage_read_one(nand_secure_storage_block_bak, item, mbuf, len);
		}
...
int nand_secure_storage_write(int item, unsigned char *buf, unsigned int len)
{
	...
	for (i = 0; i < MAX_SECURE_STORAGE_ITEM * pages_cnt_per_item; i++) {
		if ((i / pages_cnt_per_item) != item) {
			nand_physic_read_page(0, nand_secure_storage_block, i, page_size / 512, mbuf, spare);
		} else {
			MEMSET(mbuf, 0, page_size);
			if (pages_cnt_per_item == 1)
				MEMCPY(mbuf, buf, len);
			else
				MEMCPY(mbuf, buf + page_size * (i % pages_cnt_per_item), page_size);
			check_sum = nand_secure_check_sum(mbuf, len);
		...
		if (nand_physic_write_page(0, nand_secure_storage_block_bak, i, page_size / 512, mbuf, spare))
			PHY_ERR("nand_secure_storage_write page 0 fail\n");

	}

	for (i = MAX_SECURE_STORAGE_ITEM * pages_cnt_per_item; i < page_cnt_per_blk; i++) {
		page = i % (MAX_SECURE_STORAGE_ITEM * pages_cnt_per_item);
		nand_physic_read_page(0, nand_secure_storage_block_bak, page, page_size / 512, mbuf, spare);
		nand_physic_write_page(0, nand_secure_storage_block_bak, i, page_size / 512, mbuf, spare);
	}
...
int nand_secure_storage_check(void)
{
	...
	for (i = 0; i < MAX_SECURE_STORAGE_ITEM; i++) {
		ret1 = nand_secure_storage_read_one(nand_secure_storage_block, i, mbuf, page_size);
		ret2 = nand_secure_storage_read_one(nand_secure_storage_block_bak, i, mbuf, page_size);
		if (ret1 != ret2) {
			break;
		}
		if (ret1 < 0) {
