...
extern void rawnand_erase_special_block(void);
...
int uboot_info_init(struct _uboot_info *uboot)
{
	...
	for (i = aw_nand_info.boot->uboot_start_block; i < aw_nand_info.boot->uboot_next_block; i++) {
		...
		ret = is_uboot_block(i, (unsigned char *)uboot_buf);
		if (ret == 1) {
			if (blcok_offset == 0) {
				uboot_find = 1;
...
int physic_info_read(void)
{
	...
	for (start_block = UBOOT_SCAN_START_BLOCK; start_block <= UBOOT_MAX_BLOCK_NUM; start_block++) {
		ret = is_uboot_block(start_block, (unsigned char *)temp_buf);
		if (ret != 1)
			continue;
 ...
 *Note         :
 *****************************************************************************/
unsigned int  is_uboot_block(unsigned int block, unsigned char *uboot_buf)
{
	int ret = 0;
