...
int nand_phy_low_page_write_cache_set(struct _nand_physic_op_par *npo, unsigned int two_plane)
{
	int i = 0;
		...
		if (nand_phy_w_cache[i].cache_use_status == 0) {
			...
			if (nand_phy_w_cache[i].tmp_npo.mdata == NULL) {
				if (two_plane == 1)
					...
					nand_phy_w_cache[i].tmp_npo.mdata = NAND_Malloc(npo->sect_bitmap << 9);
			}
			if (nand_phy_w_cache[i].tmp_npo.sdata == NULL) {
				nand_phy_w_cache[i].tmp_npo.sdata = NAND_Malloc(npo->slen);
				NAND_Memset(nand_phy_w_cache[i].tmp_npo.sdata, 0xff, npo->slen);
			}

			if (npo->mdata) {
				if (two_plane == 1)
					NAND_Memcpy(nand_phy_w_cache[i].tmp_npo.mdata, npo->mdata, 2 * (npo->sect_bitmap << 9));
				else
					NAND_Memcpy(nand_phy_w_cache[i].tmp_npo.mdata, npo->mdata, (npo->sect_bitmap << 9));
			}

			if (npo->sdata)
				NAND_Memcpy(nand_phy_w_cache[i].tmp_npo.sdata, npo->sdata, npo->slen);
		...
		unsigned int function)
{
	...
	if ((nci->npi->operation_opt & NAND_PAIRED_PAGE_SYNC) && (function == 1)) {
		if (nand_phy_get_page_type(lnpo.page) == 2) {
			ret = nand_phy_low_page_write_cache_set(&lnpo, nsci->two_plane);
			return ret;
			...
			if (low_npo) {
				...
				ret |= m0_rw_page(low_npo, function, nsci->two_plane);
				nand_phy_low_page_write_cache_cancle(low_npo);
			} else {
				PHY_ERR("ERR! Not get low page cache when write uper page\n");
			}
		} else {
			;
		}
	} else if ((nci->npi->operation_opt & NAND_PAIRED_PAGE_SYNC) && (function == 0)) {
		if ((nand_phy_get_page_type(lnpo.page) == 2) && (nand_phy_low_page_cache_get_for_read(&lnpo, nsci->two_plane) == 0))
			return 0;
	} else {
		;
	}
