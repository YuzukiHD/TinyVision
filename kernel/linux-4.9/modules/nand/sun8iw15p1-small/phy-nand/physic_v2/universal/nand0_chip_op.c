...
int m0_read_page_start(struct _nand_physic_op_par *npo)
{
	...
	full_bitmap = ((unsigned int)1 << (ecc_block - 1)) | (((unsigned int)1 << (ecc_block - 1)) - 1);
	...
	if (ret) {
		PHY_ERR("read page start, batch cmd io send error:chip:%d block:%d page:%d sect_bitmap:%d mdata:%x slen:%d: !\n", npo->chip, npo->block, npo->page, npo->sect_bitmap, npo->mdata, npo->slen);
...
int m0_read_page_end_not_retry(struct _nand_physic_op_par *npo)
{
	...
	if (ret) {
		PHY_ERR("read page end, batch cmd io wait error:chip:%d block:%d page:%d sect_bitmap:%d mdata:%x slen:%d: !\n", npo->chip, npo->block, npo->page, npo->sect_bitmap, npo->mdata, npo->slen);
		goto ERROR;
	}
...
int m0_read_page_end(struct _nand_physic_op_par *npo)
{
	...
	if (ret == ECC_LIMIT) {
		...
		PHY_ERR("ecc err!read page, read page end error %d,chip=%d block=%d page=%d\n",
...
int m0_read_page(struct _nand_physic_op_par *npo)
{
	...
	if ((nci->nctri_chip_no >= nctri->chip_cnt) || (npo->block >= nci->blk_cnt_per_chip) || (npo->page >= nci->page_cnt_per_blk)) {
		PHY_ERR("fatal err -0, wrong input parameter, ch: %d  chip: %d/%d  block: %d/%d  page: %d/%d\n",
	...
	if ((npo->mdata == NULL) && (npo->sdata == NULL) && (npo->sect_bitmap)) {
		PHY_ERR("fatal err -1, wrong input parameter, mdata: 0x%08x  sdata: 0x%08x  sect_bitmap: 0x%x\n",
				npo->mdata, npo->sdata, npo->sect_bitmap);
		return ERR_NO_12;
	}

	if ((npo->mdata == NULL) && (npo->sdata == NULL) && (npo->sect_bitmap == 0)) {
		//PHY_DBG("warning -0, mdata: 0x%08x  sdata: 0x%08x  sect_bitmap: 0x%x\n",npo->mdata, npo->sdata, npo->sect_bitmap);
		return 0;
	}
...
int m0_write_page_start(struct _nand_physic_op_par *npo, int plane_no)
{
	...
	if ((plane_no == 0) || (plane_no == 1))
		...
		nand_read_chip_status_ready(nci);
	if ((nci->driver_no == 1) && (m1_read_retry_mode == 0x4) && (nci->retry_count != 0)) {
	...
	if (plane_no == 0) {
		...
		set_default_batch_write_cmd_seq(cmd_seq, CMD_WRITE_PAGE_CMD1, nci->opt_phy_op_par->multi_plane_write_cmd[0]);
		//set_default_batch_write_cmd_seq(cmd_seq,CMD_WRITE_PAGE_CMD1,CMD_WRITE_PAGE_CMD2);
	} else {
		//set_default_batch_write_cmd_seq(cmd_seq,0x81,CMD_WRITE_PAGE_CMD2);
		set_default_batch_write_cmd_seq(cmd_seq, nci->opt_phy_op_par->multi_plane_write_cmd[1], CMD_WRITE_PAGE_CMD2);
		//set_default_batch_write_cmd_seq(cmd_seq,CMD_WRITE_PAGE_CMD1,CMD_WRITE_PAGE_CMD2);
	}
	...
	ret = _batch_cmd_io_send(nci->nctri, cmd_seq);
	if (ret) {
		PHY_ERR("read2 page start, batch cmd io send error:chip:%d block:%d page:%d sect_bitmap:%d mdata:%x slen:%d: !\n", npo->chip, npo->block, npo->page, npo->sect_bitmap, npo->mdata, npo->slen);
		nand_disable_chip(nci);
		return ret;
	}
...
int m0_write_page_end(struct _nand_physic_op_par *npo)
{
	...
	ret = _batch_cmd_io_wait(nci->nctri, cmd_seq);
	if (ret) {
		PHY_ERR("write page end, batch cmd io wait error:chip:%d block:%d page:%d sect_bitmap:%d mdata:%x slen:%d: !\n", npo->chip, npo->block, npo->page, npo->sect_bitmap, npo->mdata, npo->slen);
	}
...
int m0_write_page(struct _nand_physic_op_par *npo)
{
	...
	if ((nci->nctri_chip_no >= nctri->chip_cnt) || (npo->block >= nci->blk_cnt_per_chip) || (npo->page >= nci->page_cnt_per_blk)) {
		PHY_ERR("wfatal err -0, wrong input parameter, ch: %d  chip: %d/%d  block: %d/%d  page: %d/%d\n",
				nctri->channel_id, nci->nctri_chip_no, nctri->chip_cnt, npo->block,
				nci->blk_cnt_per_chip, npo->page, nci->page_cnt_per_blk);
		return ERR_NO_14;
	}

	if (npo->sect_bitmap != nci->sector_cnt_per_page) {
		PHY_ERR("wFatal err -2, wrong input parameter, unaligned write page SectBitmap: 0x%x/0x%x\n",
 ...
 *Note         :
 *****************************************************************************/
int m0_read_two_plane_page_start(struct _nand_physic_op_par *npo, struct _nand_physic_op_par *npo2)
{
	int ret;
	struct _nand_chip_info *nci = nci_get_from_nsi(g_nsi, npo->chip);
	struct _nctri_cmd_seq *cmd_seq = &nci->nctri->nctri_cmd_seq;
	...
	cmd_seq->nctri_cmd[0].cmd_valid = 1;
	cmd_seq->nctri_cmd[0].cmd = nci->opt_phy_op_par->multi_plane_read_cmd[0];
	cmd_seq->nctri_cmd[0].cmd_send = 1;
	row_addr = get_row_addr(nci->page_offset_for_next_blk, npo->block, npo->page);
	if (nci->npi->operation_opt & NAND_WITH_TWO_ROW_ADR) {
		cmd_seq->nctri_cmd[0].cmd_acnt = 2;
	...
	cmd_seq->nctri_cmd[1].cmd_valid = 1;
	cmd_seq->nctri_cmd[1].cmd = nci->opt_phy_op_par->multi_plane_read_cmd[0];
	cmd_seq->nctri_cmd[1].cmd_send = 1;
	...
	cmd_seq->nctri_cmd[2].cmd = nci->opt_phy_op_par->multi_plane_read_cmd[1];
...
int m0_bad_block_check(struct _nand_physic_op_par *npo)
{
	...
	if ((nci->nctri_chip_no >= nctri->chip_cnt) || (npo->block >= nci->blk_cnt_per_chip)) {
		PHY_ERR("cfatal err -0, wrong input parameter, ch: %d  chip: %d/%d  block: %d/%d \n",
				nctri->channel_id, nci->nctri_chip_no, nctri->chip_cnt,
	...
	if (lnpo.sdata[0] != 0xff) {
		PHY_ERR("find a bad block: %d %d %d ", lnpo.chip, lnpo.block, lnpo.page);
...
int m0_bad_block_mark(struct _nand_physic_op_par *npo)
{
	...
	if ((nci->nctri_chip_no >= nctri->chip_cnt) || (npo->block >= nci->blk_cnt_per_chip)) {
		PHY_ERR("Mfatal err -0, wrong input parameter, chip: %d/%d  block: %d/%d \n",
				nctri->channel_id, nci->nctri_chip_no, nctri->chip_cnt,
				npo->block, nci->blk_cnt_per_chip);
		return ERR_NO_18;
	}
	...
	if (ret) {
		nand_free_temp_buf(mbuf, nctri->nci->sector_cnt_per_page << 9);
		PHY_ERR("bad block mark, erase block failed, blk %d, chip %d, ch %d\n",
