...
int small_nand_seed = 0;		//small nand random seed flag
...
int _generate_page_map_tab(__u32 nand_page_size, __u32 copy_cnt, __u32 page_cnt, __u32 *page_addr, __u32 *page_map_tab_addr, __u32 *tab_size)
{
	s32 i, j;
	u32 max_page_cnt;
	u32 checksum = 0;
...
int m0_write_boot0_one_pagetab_4k(unsigned char *buf, unsigned int len, unsigned int counter)
{
...
int m0_write_boot0_one_1k_mode(unsigned char *buf, unsigned int len, unsigned int counter)
{
	__u32 i, j;
	__u8  oob_buf[64];
...
int m0_write_boot0_one(unsigned char *buf, unsigned int len, unsigned int counter)
{
	struct _nand_chip_info *nci;
	__u32 pagesize = 0;
