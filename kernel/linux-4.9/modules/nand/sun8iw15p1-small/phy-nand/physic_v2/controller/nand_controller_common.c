 ...
 *Note         :
 *****************************************************************************/
struct _nand_controller_info *nctri_get(struct _nand_controller_info *head, unsigned int num)
{
	int i;
 ...
 *Note         :
 *****************************************************************************/
int start_update_nctri_interface(struct _nand_controller_info *nctri, u32 input_timing_para, u32 ddr_type, u32 ddr_type_old, u32 sclk0, u32 sclk1, u32 sdr_edo_i, u32 ddr_edo_i, u32 ddr_delay_i)
{
	s32 ret;
...
#endif
	} else if (((ddr_type_old_bak == ONFI_DDR2) || (ddr_type_old_bak == TOG_DDR2)) && (ddr_type_bak == SDR)) {
#if 0
		if (SUPPORT_DDR2_SPECIFIC_CFG) {
	...
	if ((id_number_ctl & 0x01) != 0) {//bit 0, set freq para
		...
		if (((para & 0xffffff) == nctri->nci->npi->id_number) || ((para & 0xffffff) == 0xeeeeee)) {
			...
			if ((script_frequence > 10) && (script_frequence < 100)) {
				...
				PHY_DBG("_update_ext accessfreq_para: update freq from script, %d\n", script_frequence);
			} else {
		...
		if ((script_frequence > 10) && (script_frequence < 100)) {
			...
			PHY_DBG("_update_ext accessfreq_para: update freq high, %d\n", script_frequence);
		} else {
			PHY_ERR("_update_ext accessfreq_para: wrong freq, %d\n", script_frequence);
			return ERR_NO_42;
		}
