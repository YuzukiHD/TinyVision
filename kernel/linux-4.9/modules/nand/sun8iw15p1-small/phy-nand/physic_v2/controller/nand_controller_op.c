...
extern void ndfc_dma_config_start(struct _nand_controller_info *nctri, u8 rw, void *buff_addr, u32 len);
extern s32 ndfc_wait_dma_end(struct _nand_controller_info *nctri, u8 rw, void *buff_addr, u32 len);
...
void ndfc_print_random(struct _nand_controller_info *nctri)
{
	...
	for (i = 0; i < 8; i++) {
		PHY_DBG("*****reg_random_seed_x+%d(0x%08x): 0x%08x*****\n",
				i, (nctri->nreg.reg_random_seed_x + i),
 ...
 *Note         :
 *****************************************************************************/
void ndfc_enable_ecc(struct _nand_controller_info *nctri, u32 pipline, u32 randomize)
{
...
{
	s32 ret;
	ret = wait_reg_status(nctri->nreg.reg_sta, NDFC_CMD_INT_FLAG, NDFC_CMD_INT_FLAG, 0xffffff);//songwj
	if (ret != 0) {
...
{
	*(nctri->nreg.reg_ctl) = ((*(nctri->nreg.reg_ctl)) & (~NDFC_PAGE_SIZE)) | ((page_size & 0xf) << 8);
	return 0;
}
...
void ndfc_set_new_rand_seed(struct _nand_controller_info *nctri, u32 page_no)
{
	...
	for (i = 0, j = 0; i < 16; i++, j++) {
		...
		*(nctri->nreg.reg_random_seed_x + i) =  random_seed_new[index + j];
	}
}
 ...
 *Description  :
 *Parameter	  :	0: BCH & 1KB Frame_Size & Random_OLD; 1: LDPC & 2KB Frame_Size & Random_NEW
 *Return 	  : NULL
 *Note		  :
 *****************************************************************************/
...
{
	return ((*nctri->nreg.reg_sta & NDFC_RB_B2R) && (*nctri->nreg.reg_int & NDFC_B2R_INT_ENABLE));
}
...
{
	return ((*nctri->nreg.reg_sta & NDFC_CMD_INT_FLAG) && (*nctri->nreg.reg_int & NDFC_CMD_INT_ENABLE));
}
...
{
	return ((*nctri->nreg.reg_sta & NDFC_DMA_INT_FLAG) && (*nctri->nreg.reg_int & NDFC_DMA_INT_ENABLE));
}
...
s32 ndfc_wait_rb_ready(struct _nand_controller_info *nctri, u32 rb)
{
	...
	ret = wait_reg_status(nctri->nreg.reg_sta, rb_check, rb_check, 0xffffff);
...
s32 save_nctri(struct _nand_controller_info *nctri)
{
	...
	nctri->nreg_bak.reg_data_pattern_sta = *nctri->nreg.reg_data_pattern_sta;
	...
	nctri->nreg_bak.reg_user_data_len_base = *nctri->nreg.reg_user_data_len_base;
	...
	nctri->nreg_bak.reg_cmd_repeat_interval = *nctri->nreg.reg_cmd_repeat_interval;
	...
	nctri->nreg_bak.reg_valid_data_dma_cnt = *nctri->nreg.reg_valid_data_dma_cnt;
	nctri->nreg_bak.reg_data_dma_base = *nctri->nreg.reg_data_dma_base;
	nctri->nreg_bak.reg_data_dma_size_2x = *nctri->nreg.reg_data_dma_size_2x;
	...
	nctri->nreg_bak.reg_emce_iv_fac_cmp_val = *nctri->nreg.reg_emce_iv_fac_cmp_val;
	nctri->nreg_bak.reg_emce_iv_cal_fac_base = *nctri->nreg.reg_emce_iv_cal_fac_base;
	...
	nctri->nreg_bak.reg_enc_ldpc_mode_set = *nctri->nreg.reg_enc_ldpc_mode_set;
	nctri->nreg_bak.reg_cor_ldpc_mode_set = *nctri->nreg.reg_cor_ldpc_mode_set;
	...
	nctri->nreg_bak.reg_cmd_descr_base_addr = *nctri->nreg.reg_cmd_descr_base_addr;
	...
	nctri->nreg_bak.reg_csic_bist_ctl_reg = *nctri->nreg.reg_csic_bist_ctl_reg;
...
s32 ndfc_change_page_size(struct _nand_controller_info *nctri)
{
	...
	*nctri->nreg.reg_spare_area = (nci->sector_cnt_per_page << 9);  // spare area
 ...
 *Note         :
 *****************************************************************************/
s32 _normal_cmd_io_send(struct _nand_controller_info *nctri, struct _nctri_cmd *icmd)
{
	s32 ret = 0;
	u32 cmd_val = 0;
 ...
 *Note         :
 *****************************************************************************/
s32 _normal_cmd_io_wait(struct _nand_controller_info *nctri, struct _nctri_cmd *icmd)
{
	s32 ret = 0;
	u32 rb = ndfc_get_selected_rb_no(nctri);
	//u32 reg_val;
 ...
 *Note         :
 *****************************************************************************/
s32 _batch_cmd_io_send(struct _nand_controller_info *nctri, struct _nctri_cmd_seq *cmd_list)
{
	s32 ret = 0;
 ...
 *Note         :
 *****************************************************************************/
s32 _batch_cmd_io_wait(struct _nand_controller_info *nctri, struct _nctri_cmd_seq *cmd_list)
{
	s32 ret = 0;
 ...
 *Note         :
 *****************************************************************************/
s32 ndfc_execute_cmd(struct _nand_controller_info *nctri, struct _nctri_cmd_seq *cmd_list)
{
	s32 ret = 0;
	s32 c = 0;
	s32 rb;
 ...
 *Note         :
 *****************************************************************************/
s32 ndfc_get_spare_data(struct _nand_controller_info *nctri, u8 *sbuf, u32 udata_cnt)
{
	s32 i;
 ...
 *Note         :
 *****************************************************************************/
s32 ndfc_set_spare_data(struct _nand_controller_info *nctri, u8 *sbuf, u32 udata_cnt)
{
	s32 i;
	...
	for (i = 0; i < udata_cnt; i = i + 4) {
		*(nctri->nreg.reg_user_data_base + (i >> 2)) = sbuf[i + 3] << 24 | sbuf[i + 2] << 16 | sbuf[i + 1] << 8 | sbuf[i];
	}
	return 0;
}
 ...
 *Note         : udata_cnt is 4 bytes aligned
 *****************************************************************************/
s32 ndfc_set_user_data_len_cfg(struct _nand_controller_info *nctri, u32 udata_cnt)
{
 ...
 *Note         : udata_cnt is 4 bytes aligned
 *****************************************************************************/
s32 ndfc_set_user_data_len_cfg_4bytesper1k(struct _nand_controller_info *nctri, u32 udata_cnt)
{
 ...
 *Note         :
 *****************************************************************************/
void ndfc_get_nand_interface(struct _nand_controller_info *nctri, u32 *pddr_type, u32 *psdr_edo, u32 *pddr_edo, u32 *pddr_delay)
{
	u32 cfg = 0;
 ...
 *Note         :
 *****************************************************************************/
s32 ndfc_change_nand_interface(struct _nand_controller_info *nctri, u32 ddr_type, u32 sdr_edo, u32 ddr_edo, u32 ddr_delay)
{
	u32 cfg = 0;
 ...
 *Note         :
 *****************************************************************************/
void ndfc_emce_ctrl_setting(struct _nand_controller_info *nctri, u32 enable_flag)
{
	if (enable_flag)
 ...
 *Note         :
 *****************************************************************************/
void ndfc_emce_fac_compare_value_setting(struct _nand_controller_info *nctri, u32 value)
{
 ...
 *Note         :
 *****************************************************************************/
void ndfc_emce_calculate_factor_setting_for_serial_addr(struct _nand_controller_info *nctri, u32 sector_addr, u32 sector_mapping)
{
	int i = 0;
	for (i = 0; i < 32; i++) {
		if ((sector_mapping >> i) & 0x01) {
			...
			*nctri->nreg.reg_emce_iv_cal_fac_base = *nctri->nreg.reg_emce_iv_fac_cmp_val;
		}
	}
 ...
 *Note         :
 *****************************************************************************/
void ndfc_emce_calculate_factor_setting(struct _nand_controller_info *nctri, u32 *values, u32 offset_start, u32 lenth)
{
	int i = offset_start;
