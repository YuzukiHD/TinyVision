...
int ndfc_dma_config_start(struct _nand_controller_info *nctri, u8 rw, void *buff_addr, u32 len)
{
	...
	if (nctri->dma_type == 1) {//MBUS DMA
		...
		if (nctri->dma_addr & 0x1f) {	//zhouk for DMA addr must be 32B aligned
			PHY_ERR("ndfc_dma_config_start: buff addr(0x%x) is not 32Bytes aligned", nctri->dma_addr);
		}
...
dma_end:
	...
	NAND_InvaildDCacheRegion(rw, (void *)(unsigned long)nctri->dma_addr, len);
...
s32 ndfc_check_read_data_sta(struct _nand_controller_info *nctri, u32 eblock_cnt)
{
	...
	if (*nctri->nreg.reg_data_pattern_sta == ((0x1U<<(eblock_cnt-1)) | ((0x1U<<(eblock_cnt-1))-1))) {
		...
		//PHY_DBG("!!!!read data sta, only some ecc blocks are all '1' or all '1', 0x%x 0x%x\n", *nctri->nreg.reg_ecc_sta, *nctri->nreg.reg_pat_id);
		return 1;
	}
}
