...
typedef struct {
	...
	unsigned int        random_cmd2_send_flag;				//special nand cmd for some nand in batch cmd, only for write
...
struct _optional_phy_op_par {
	unsigned char        multi_plane_read_cmd[2];               //the command for multi-plane read, the sequence is [0] -ADDR- [0] -ADDR- [1] - DATA
	unsigned char        multi_plane_write_cmd[2];              //the command for multi-plane program, the sequence is 80 -ADDR- DATA - [0] - [1] -ADDR- DATA - 10/15
	unsigned char        multi_plane_copy_read_cmd[3];           //the command for multi-plane page copy-back read, the sequence is [0] -ADDR- [1] -ADDR- [2]
	unsigned char        multi_plane_copy_write_cmd[3];          //the command for multi-plane page copy-back program, the sequence is [0] -ADDR- [1] - [2] -ADDR- 10
	unsigned char        multi_plane_status_cmd;                //the command for multi-plane operation status read, the command may be 0x70/0x71/0x78/...
	unsigned char        inter_bnk0_status_cmd;                 //the command for inter-leave bank0 operation status read, the command may be 0xf1/0x78/...
	unsigned char        inter_bnk1_status_cmd;                 //the command for inter-leave bank1 operation status read, the command may be 0xf2/0x78/...
	unsigned char        bad_block_flag_position;               //the flag that marks the position of the bad block flag,0x00-1stpage/ 0x01-1st&2nd page/ 0x02-last page/ 0x03-last 2 page
	unsigned short       multi_plane_block_offset;              //the value of the block number offset between the left-plane block and the right pane block
};
...
struct __RAWNandStorageInfo_t {
	...
	unsigned int        random_addr_num;					//random col addr num in batch cmd
	unsigned int        nand_real_page_size;				//real physic page size
	struct _optional_phy_op_par OptPhyOpPar;        //the parameters for some optional operation
};
...
struct _nand_phy_info_par {
	...
	unsigned int       die_cnt_per_chip;                      //the count of the die in one nand flash chip
	unsigned int       sect_cnt_per_page;                     //the count of the sectors in one single physical page
	unsigned int       page_cnt_per_blk;                      //the count of the pages in one single physical block
	unsigned int       blk_cnt_per_die;                       //the count fo the physical blocks in one nand flash die
	unsigned int       operation_opt;                       //the bitmap that marks which optional operation that the nand flash can support
	unsigned int       valid_blk_ratio;                      //no use
	unsigned int       access_freq;                         //the highest access frequence of the nand flash chip, based on mhz
	unsigned int       ecc_mode;                            //the ecc mode for the nand flash chip, 0: bch-16, 1:bch-28, 2:bch_28 3:32 4:40 5:48 6:56 7:60 8:64 9:72
	unsigned int       read_retry_type;                      //related to driver_no
	unsigned int       ddr_type;                            //0:sdr 1:null 2:onfi nvddr1 3:toggle ddr1 0x12: onfi ddr2 0x13: toggle ddr2
	unsigned int       option_phyisc_op_no;                 //the pointer point to the optional operation parameter
...
struct _nand_physic_function {
	...
	int (*nand_read_boot0_page)(struct _nand_chip_info *nci, struct _nand_physic_op_par *npo);
	int (*nand_write_boot0_page)(struct _nand_chip_info *nci, struct _nand_physic_op_par *npo);
...
struct _nand_chip_info {
	...
	unsigned int       sector_cnt_per_page;                //the count of sectors in one single physic page, one sector is 0.5k
	unsigned int       page_cnt_per_blk;                   //the count of physic pages in one physic block
	unsigned int       page_offset_for_next_blk;           //the count of physic pages in one physic block
	...
	unsigned int       ecc_mode;                                 //the Ecc Mode for the nand flash chip, 0: bch-16, 1:bch-28, 2:bch_32
	unsigned int       max_erase_times;
	unsigned int       driver_no;

	unsigned int       interface_type;   //0x0: sdr; 0x2: nvddr; 0x3: tgddr; 0x12: nvddr2; 0x13: tgddr2
	unsigned int       frequency;        //the parameter of the hardware access clock, based on 'MHz'
	unsigned int       timing_mode;      //current timing mode

	unsigned int       support_change_onfi_timing_mode; //nand flash support to change timing mode according to ONFI 3.0
	unsigned int       support_ddr2_specific_cfg;                //nand flash support to set ddr2 specific feature according to ONFI 3.0 or Toggle DDR2
	unsigned int       support_io_driver_strength;      //nand flash support to set io driver strength according to ONFI 2.x/3.0 or Toggle DDR1/DDR2
	unsigned int       support_vendor_specific_cfg;     //nand flash support to set vendor's specific configuration
	unsigned int       support_onfi_sync_reset;         //nand flash support onfi's sync reset
	unsigned int       support_toggle_only;             //nand flash support toggle interface only, and do not support switch between legacy and toggle
	...
	struct _optional_phy_op_par *opt_phy_op_par;        //the parameters for some optional operation
...
struct _nand_super_chip_info {
	...
	unsigned int       sector_cnt_per_super_page;                //the count of sectors in one single physic page, one sector is 0.5k
	unsigned int       page_cnt_per_super_blk;                   //the count of physic pages in one physic block
	unsigned int       page_offset_for_next_super_blk;           //the count of physic pages in one physic block
	...
	struct _nand_chip_info *d_channel_nci_1;                   //first nci in dual_channel operation
	struct _nand_chip_info *d_channel_nci_2;                   //second nci in dual_channel operation
...
/* [useless] nand flash support internal inter-leave operation, it based multi-bank */
#define NAND_INT_INTERLEAVE (1 << 5)
/* [useless] nand flash support external inter-leave operation, it based multi-chip */
