...
extern int clean_physic_info(void);
extern u32 NAND_GetPageNo(u32 lsb_page_no);
extern __u32 NAND_GetLsbPages(void);
extern __u32 NAND_GetLsbblksize(void);
extern __u32 NAND_GetPhyblksize(void);
...
int rawnand_write_uboot_one(unsigned char *buf, unsigned int len, struct _boot_info *info_buf, unsigned int info_len, unsigned int counter)
{
	...
	if (((0 == nci->lsb_page_type) && (real_len <= NAND_GetPhyblksize())) || ((0 != nci->lsb_page_type) && (real_len <= NAND_GetLsbblksize()))) {
...
int rawnand_read_uboot_one(unsigned char *buf, unsigned int len, unsigned int counter)
{
	...
	if (((0 == nci->lsb_page_type) && (len <= NAND_GetPhyblksize())) || ((0 != nci->lsb_page_type) && (len <= NAND_GetLsbblksize()))) {
...
int RAWNAND_UpdatePhyArch(void)
{
	...
	if ((id_number_ctl&0x0e) != 0) {
		...
		if ((para != 0xffffffff) && (id_number_ctl & 0x04)) {//get script success
			if (((para & 0xffffff) == g_nctri->nci->npi->id_number) || ((para & 0xffffff) == 0xeeeeee)) {
		...
		if ((para != 0xffffffff) && (id_number_ctl & 0x08)) {//get script success
			if (((para & 0xffffff) == g_nctri->nci->npi->id_number) || ((para & 0xffffff) == 0xeeeeee)) {
