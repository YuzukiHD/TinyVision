 ...
 *Note         :
 *****************************************************************************/
int nsci_add_to_nssi(struct _nand_super_storage_info *nssi, struct _nand_super_chip_info *node)
{
 ...
 *Note         :
 *****************************************************************************/
struct _nand_super_chip_info *nsci_get_from_nssi(struct _nand_super_storage_info *nssi, unsigned int num)
{
	int i;
	struct _nand_super_chip_info *nsci;
...
int get_nand_structure(struct _nand_super_storage_info *nssi)
{
	...
	if ((ret == 0) || (ret == 2)) {
		...
		if (nand_permanent_data.magic_data == MAGIC_DATA_FOR_PERMANENT_DATA) {
			...
			nand_storage = (struct __RAWNandStorageInfo_t *)(&nand_permanent_data);
 ...
 *Note         :
 *****************************************************************************/
int init_nsci_from_nctri(struct _nand_super_storage_info *nssi, struct _nand_super_chip_info *nsci, struct _nand_controller_info *nctri, unsigned int channel_num, unsigned int chip_no, unsigned int nsci_num_in_nctri)
{
	int rb0 = 0, rb1 = 0;
	...
	if ((nssi->support_two_plane != 0) && (nsci->nci_first->npi->operation_opt & NAND_MULTI_PROGRAM) && (nsci->nci_first->opt_phy_op_par->multi_plane_block_offset == 1)) {
		nsci->two_plane = 1;
	} else {
		nsci->two_plane = 0;
	}
	...
	nsci->nand_physic_erase_super_block = nand_physic_function[nsci->driver_no].nand_physic_erase_super_block;
	nsci->nand_physic_read_super_page = nand_physic_function[nsci->driver_no].nand_physic_read_super_page;
	nsci->nand_physic_write_super_page = nand_physic_function[nsci->driver_no].nand_physic_write_super_page;
	nsci->nand_physic_super_bad_block_check = nand_physic_function[nsci->driver_no].nand_physic_super_bad_block_check;
	nsci->nand_physic_super_bad_block_mark = nand_physic_function[nsci->driver_no].nand_physic_super_bad_block_mark;
 ...
 *****************************************************************************/
int nand_build_nssi(struct _nand_super_storage_info *nssi, struct _nand_controller_info *nctri)
{
	int rb0, rb1, rb2, ret = 0;
	...
	for (i = 0; i < nsci_num; i++) {
		//nsci = (struct _nand_super_chip_info *)MALLOC(sizeof(struct _nand_super_chip_info));
		nsci = &nsci_data[i];
		if (nsci == NULL) {
		...
		init_nsci_from_nctri(nssi, nsci, nctri, channel_num, i, nsci_num_in_nctri);
...
int nand_build_storage_info(void)
{
	...
	MEMCPY(&g_nand_storage_info->OptPhyOpPar, g_nsi->nci->opt_phy_op_par, sizeof(struct _optional_phy_op_par));
