...
int nand_first_read_id(struct _nand_chip_info *nci, unsigned int chip_no, unsigned char *id)
{
	int ret = 0;
	struct _nctri_cmd_seq *cmd_seq = &nci->nctri->nctri_cmd_seq;
 ...
 *Note         :
 *****************************************************************************/
s32 _change_nand_onfi_timing_mode(struct _nand_chip_info *nci, u32 if_type, u32 timing_mode)
{
	u8 addr;
	u8 p[4];
...
s32 _setup_nand_toggle_vendor_specific_feature(struct _nand_chip_info *nci)
{
	...
	if ((nci->id[0] == 0x45) || (nci->id[0] == 0x98)) {// sandisk/toshiba nand flash
...
s32 _setup_ddr_nand_force_to_sdr_para(struct _nand_chip_info *nci)
{
	...
	if ((nci->id[0] == 0x45) || (nci->id[0] == 0x98)) {// sandisk/toshiba nand flash
		...
		nand_get_feature(nci, &addr, pr);
		//PHY_ERR("get feature(addr 0x80) 0x%x 0x%x 0x%x 0x%x!\n",pr[0],pr[1],pr[2],pr[3]);
		if (pr[0] == 0x00) {
 ...
 *Note         :
 *****************************************************************************/
s32 _change_all_nand_parameter(struct _nand_controller_info *nctri, u32 ddr_type, u32 pre_ddr_type, u32 dclk)
{
	...
	while (nci) {
		...
		if (ddr_change_mode == 1) {
			...
			if ((nci->support_ddr2_specific_cfg) && (ddr_type == ONFI_DDR2)) {
			...
			if ((nci->support_ddr2_specific_cfg) && (pre_ddr_type == ONFI_DDR2)) {
				ret = _setup_nand_onfi_ddr2_para(nci);
				if (ret != 0) {
					PHY_ERR("_setup_nand_onfi_ddr2_para() failed! %d\n", ddr_change_mode);
					return ret;
			...
			if ((nci->support_ddr2_specific_cfg) && (pre_ddr_type == TOG_DDR2)) {
			...
			if (ddr_type == SDR) {
				...
				if ((nci->support_ddr2_specific_cfg) && (ddr_type == TOG_DDR2)) {
					ret = _setup_nand_toggle_ddr2_para(nci);
					if (ret) {
 ...
 *Note         :
 *****************************************************************************/
s32 _get_right_timing_para(struct _nand_controller_info *nctri, u32 ddr_type, u32 *good_sdr_edo, u32 *good_ddr_edo, u32 *good_ddr_delay)
{
	...
	for (edo = 0; edo < edo_cnt; edo++) {
		...
		for (delay = 0; delay < delay_cnt; delay += 2) {
			...
			if ((nctri->nci->id[0] == tmpChipID[0]) && (nctri->nci->id[1] == tmpChipID[1]) && (nctri->nci->id[2] == tmpChipID[2]) && (nctri->nci->id[3] == tmpChipID[3])) {
				PHY_DBG("ddr edo:0x%x, delay:0x%x is ", edo, ddr_delay);
 ...
 *****************************************************************************/
int set_cmd_with_nand_bus(struct _nand_chip_info *nci, u8 *cmd, u32 wait_rb, u8 *addr, u8 *dat, u32 dat_len, u32 counter)
{
	int ret, i = 0;
	u8 *p_dat = dat;
 ...
 *Note         : one cmd; one addr; some data
 *****************************************************************************/
int get_data_with_nand_bus_one_cmd(struct _nand_chip_info *nci, u8 *cmd, u8 *addr, u8 *dat, u32 dat_len)
{
	int ret, j;
	u8 *p_dat = dat;
 ...
 *Note         :
 *****************************************************************************/
int get_data_block_cnt_for_boot0_ecccode(struct _nand_chip_info *nci, u8 ecc_mode)
{
