...
extern int nand_physic_write_page(unsigned int chip, unsigned int block, unsigned int page, unsigned int bitmap, unsigned char *mbuf, unsigned char *sbuf);
...
int nand_write_data_in_whole_block(unsigned int chip, unsigned int block, unsigned char *mbuf, unsigned int mlen, unsigned char *sbuf, unsigned int slen)
{
	...
	ret = nci->nand_physic_erase_block(&npo);
	if (ret != 0) {
		PHY_ERR("nand_write_data_in block error1 chip:0x%x block:0x%x \n", chip, block);
		return ret;
	}
	...
	for (i = 0; i < nci->page_cnt_per_blk; i++) {
		...
		if (ret == 0) {
			...
			PHY_ERR("nand_write_data_in block error2 chip:0x%x block:0x%x page:0x%x \n", chip, block, i);
		}
	}
...
int nand_read_data_in_whole_block(unsigned int chip, unsigned int block, unsigned char *mbuf, unsigned int mlen, unsigned char *sbuf, unsigned int slen)
{
	...
	for (i = 0; i < nci->page_cnt_per_blk; i++) {
		...
		if (ret >= 0) {
			...
			PHY_ERR("nand_read_data_in block error2 chip:0x%x block:0x%x page:0x%x \n", chip, block, i);
		}
	}
