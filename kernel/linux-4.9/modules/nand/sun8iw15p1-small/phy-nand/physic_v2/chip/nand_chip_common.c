 ...
 *Note         :
 *****************************************************************************/
int nci_add_to_nctri(struct _nand_controller_info *nctri, struct _nand_chip_info *node)
{
	struct _nand_chip_info *nci;
 ...
 *Note         :
 *****************************************************************************/
struct _nand_chip_info *nci_get_from_nctri(struct _nand_controller_info *nctri, unsigned int num)
{
	int i;
	struct _nand_chip_info *nci;
 ...
 *Note         :
 *****************************************************************************/
struct _nand_chip_info *nci_get_from_nsi(struct _nand_storage_info *nsi, unsigned int num)
{
	int i;
	struct _nand_chip_info *nci;
 ...
 *Note         :
 *****************************************************************************/
struct _nand_phy_info_par *search_id(unsigned char *id)
{
	int i = 0, j = 0, ok_bytes = 0, ok_ptr = 0, nums = 0;
	...
	for (j = 0; id_ptr->nand_id[0] != 0xff; j++, id_ptr++) {
		for (i = 0, nums = 0; i < 6; i++) {
			if (!((id_ptr->nand_id[i] == id[i]) || (id_ptr->nand_id[i] == 0xff)))
				break;
 ...
 *Note         :
 *****************************************************************************/
int init_nci_from_id(struct _nand_chip_info *nci, struct _nand_phy_info_par *npi)
{
	...
	nci->timing_mode = 0x0;
	nci->support_change_onfi_timing_mode = (npi->operation_opt & NAND_TIMING_MODE) ? 1 : 0;
	nci->support_ddr2_specific_cfg = (npi->operation_opt & NAND_DDR2_CFG) ? 1 : 0;
	nci->support_io_driver_strength = (npi->operation_opt & NAND_IO_DRIVER_STRENGTH) ? 1 : 0;
	nci->support_vendor_specific_cfg = (npi->operation_opt & NAND_VENDOR_SPECIFIC_CFG) ? 1 : 0;
	nci->support_onfi_sync_reset = (npi->operation_opt & NAND_ONFI_SYNC_RESET_OP) ? 1 : 0;
	...
	nci->nand_physic_erase_block = nand_physic_function[nci->driver_no].nand_physic_erase_block;
	nci->nand_physic_read_page = nand_physic_function[nci->driver_no].nand_physic_read_page;
	nci->nand_physic_write_page = nand_physic_function[nci->driver_no].nand_physic_write_page;
	nci->nand_physic_bad_block_check = nand_physic_function[nci->driver_no].nand_physic_bad_block_check;
	nci->nand_physic_bad_block_mark = nand_physic_function[nci->driver_no].nand_physic_bad_block_mark;
	nci->nand_read_boot0_page  = nand_physic_function[nci->driver_no].nand_read_boot0_page;
	nci->nand_write_boot0_page  = nand_physic_function[nci->driver_no].nand_write_boot0_page;
	nci->nand_read_boot0_one  = nand_physic_function[nci->driver_no].nand_read_boot0_one;
	nci->nand_write_boot0_one  = nand_physic_function[nci->driver_no].nand_write_boot0_one;

	nci->is_lsb_page = nand_physic_function[nci->driver_no].is_lsb_page;

	nand_physic_special_init = nand_physic_function[nci->driver_no].nand_physic_special_init;
	nand_physic_special_exit = nand_physic_function[nci->driver_no].nand_physic_special_exit;


	//update according to external configuration
	if ((!SUPPORT_CHANGE_ONFI_TIMING_MODE) || (nci->support_onfi_sync_reset == 0)) {
 ...
 *Note         :
 *****************************************************************************/
int nand_build_nsi(struct _nand_storage_info *nsi, struct _nand_controller_info *nctri)
{
	int channel, chips, i;
	...
	for (i = 0; i < 2; i++) {
		for (channel = 0; channel < MAX_CHANNEL; channel++) {
			...
			for (chips = 0; chips < MAX_CHIP_PER_CHANNEL; chips++) {
				...
				if (nand_first_reset_chip(&lnci, chips) != NAND_OP_TRUE) {
					PHY_ERR("nand_reset_chip error %d\n", lnci.chip_no);
					break;
				}

				if (nand_first_read_id(&lnci, chips, id) != NAND_OP_TRUE) {
				...
				if (nand_id_tbl == NULL) {
					PHY_DBG("nand not support! channel %d chip %d: %02x %02x %02x %02x %02x %02x %02x %02x\n",
							channel, chips, id[0], id[1], id[2], id[3], id[4], id[5], id[6], id[7]);
					continue;
				} else {
					PHY_DBG("channel %d chip %d: %02x %02x %02x %02x %02x %02x %02x %02x\n",
							channel, chips, id[0], id[1], id[2], id[3], id[4], id[5], id[6], id[7]);
				}

				/*L05B 3D nand, Vccq = 1.8V, need fix vcc*/
				if (id[0] == 0x2c && id[1] == 0x84 && id[2] == 0x44 && id[3] == 0x32 && id[4] == 0xaa && id[5] == 0x04)
					NAND_3DNand_Request();
				if (init_nci_from_id(&lnci, nand_id_tbl) != NAND_OP_TRUE) {
					PHY_ERR("init_nci_from_id error %d\n", lnci.chip_no);
					break;
				}

				//nci = (struct _nand_chip_info *)MALLOC(sizeof(struct _nand_chip_info));
			...
			if (oob[0] == 0x00) {
				if (ret >= 0) {
					if ((ret >= 0) && (oob[1] == 0x78) && (oob[2] == 0x69) && (oob[3] == 0x87) && (oob[4] == 0x41) && (oob[5] == 0x52) && (oob[6] == 0x43) && (oob[7] == 0x48)) {
						...
						PHY_DBG("search nand structure %d  %d: get last physic arch ok 0x%x 0x%x!\n", npo.block, npo.page, parch->support_two_plane, parch->support_vertical_interleave);
						ret2 = 0;
						goto search_end;
					}
					if ((ret >= 0) && (oob[1] == 0x50) && (oob[2] == 0x48) && (oob[3] == 0x59) && (oob[4] == 0x41) && (oob[5] == 0x52) && (oob[6] == 0x43) && (oob[7] == 0x48)) {
						MEMCPY(phy_arch, parch, sizeof(struct _nand_permanent_data));
						PHY_DBG("search nand structure %d  %d: get old physic arch ok 0x%x 0x%x!\n", npo.block, npo.page, parch->support_two_plane, parch->support_vertical_interleave);
						ret2 = 2;
						goto search_end;
					}
				} else {
					PHY_DBG("search nand structure: bad block no physic arch info!\n");
				}
			} else if (oob[0] == 0xff) {
				if ((ret >= 0) && (i == 0)) {
					PHY_DBG("search nand structure: find a good block: %d, but no physic arch info.\n", npo.block);
					ret2 = 1;
					goto search_end;
				} else {
					PHY_DBG("search nand structure: blank block!\n");
				}
			} else {
				PHY_DBG("search nand structure: unkonwn 0x%x!\n", oob[0]);
			}
		}
	}
...
int write_nand_structure(void *phy_arch, unsigned int good_blk_no, unsigned int blk_cnt)
{
	...
	for (b = good_blk_no; b < good_blk_no+blk_cnt; b++) {
		...
		if (ret < 0) {
			PHY_ERR("write nand structure: erase chip %d, block %d error\n", npo.chip, npo.block);
			MEMSET(oob, 0, 64);
			...
			for (p = 0; p < g_nctri->nci->page_cnt_per_blk; p++) {
				...
				if (ret < 0) {
					PHY_ERR("write nand structure: mark bad block, write chip %d, block %d, page %d error\n", npo.chip, npo.block, npo.page);
				}
			}
		} else {
			...
			for (ok = 0, p = 0; p < g_nctri->nci->page_cnt_per_blk; p++) {
				...
				nand_wait_all_rb_ready();
				if (ret != 0) {
					PHY_ERR("write nand structure: write chip %d, block %d, page %d error\n", npo.chip, npo.block, npo.page);
				} else {
					if (p < retry) {
						ok = 1;
					}
...
int set_nand_structure(void *phy_arch)
{
	...
	if (ret == 1) {
		...
		PHY_ERR("search nand structure: can not find good block: 12~112\n");
	...
	ret = read_nand_structure(data, &good_blk_no);
	if (ret != 0) {
		PHY_ERR("write nand structure: can not find nand structure: 12~112\n");
	}
