...
extern int _get_read_retry_table(struct _nand_chip_info *nci, u8 *rr_tab);
extern void ndfc_channel_select(struct _nand_controller_info *nctri, u32 channel_sel);
extern void ndfc_enable_ldpc_ecc(struct _nand_controller_info *nctri, u32 pipline);
extern void ndfc_set_new_rand_seed(struct _nand_controller_info *nctri, u32 page_no);
extern __u32 rawnand_get_twoplane_flag(void);
extern void ndfc_enable_encode(struct _nand_controller_info *nctri);
extern void ndfc_enable_decode(struct _nand_controller_info *nctri);
extern int before_update_nctri_interface(struct _nand_controller_info *nctri);
extern int update_boot0_nctri_interface(struct _nand_controller_info *nctri);

s32 m8_read_boot0_page_cfg_mode(struct _nand_chip_info *nci, struct _nand_physic_op_par *npo, struct boot_ndfc_cfg cfg)
{
	...
	bitmap = ((u32)1 << (cfg.page_size_kb - 1)) | (((u32)1 << (cfg.page_size_kb - 1)) - 1);
...
s32 m8_write_boot0_page_cfg_mode(struct _nand_chip_info *nci, struct _nand_physic_op_par *npo, struct boot_ndfc_cfg cfg)
{
	uchar spare[64];
	struct _nctri_cmd_seq *cmd_seq = &nci->nctri->nctri_cmd_seq;
	u32 row_addr = 0, col_addr = 0, blk_mask, config = 0, i;
 ...
 *Note         :
 *****************************************************************************/
int m8_read_boot0_page(struct _nand_chip_info *nci, struct _nand_physic_op_par *npo)
{
 ...
 *Note         :
 *****************************************************************************/
int m8_write_boot0_page(struct _nand_chip_info *nci, struct _nand_physic_op_par *npo)
{
	struct boot_ndfc_cfg cfg;
...
int m8_read_boot0_one_pagetab_4k(unsigned char *buf, unsigned int len, unsigned int counter)
{
...
int m8_read_boot0_one_1k_mode(unsigned char *buf, unsigned int len, unsigned int counter)
{
	__u32 i, j;
	__u8  oob_buf[64];
	...
	struct _nand_chip_info *nci = NULL;
	struct _nand_physic_op_par lnpo;
...
int m8_read_boot0_one(unsigned char *buf, unsigned int len, unsigned int counter)
{
	struct _nand_chip_info *nci;
	__u32 pagesize = 0;
...
int m8_write_boot0_one(unsigned char *buf, unsigned int len, unsigned int counter)
{
	...
	for (j = 0; j < copies_per_block; j++) {
		...
		for (k = 0; k < NAND_BOOT0_PAGE_CNT_PER_COPY_2; k++) {
			...
			if (nci->is_lsb_page(lnpo.page)) {
				...
				nand_wait_all_rb_ready();
				if (m1_write_page_FF(&lnpo, (nci->sector_cnt_per_page / 2)) != 0) {
					PHY_ERR("Warning. Fail in writing page %d in block %d.\n", lnpo.page, lnpo.block);
				}
			}
		}
