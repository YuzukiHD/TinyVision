...
extern int add_new_bad_block(struct _nand_info *nand_info,
		unsigned short nDieNum, unsigned short nBlkNum);
extern unsigned int PHY_VirtualBadBlockMark(unsigned int nDieNum,
...
int nand_mark_bad_blk(struct _nand_partition *nand, struct _physic_par *p)
{
	...
	//	ret =
	//nand->phy_partition->page_write(super_block.Chip_NO,super_block.Block_NO,0,nand->full_bitmap,nand->phy_partition->new_bad_block,spare);
	//	if(ret != 0)
	//	{
