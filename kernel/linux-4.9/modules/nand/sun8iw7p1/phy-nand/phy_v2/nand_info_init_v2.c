...
void print_boot_info(struct _boot_info *boot);
...
extern int read_mbr_partition_v3(struct _nand_info *nand_info,
		unsigned int chip, unsigned int start_block);
extern int read_factory_block_v3(struct _nand_info *nand_info,
		unsigned int chip, unsigned int start_block);
/*****************************************************************************
 *Name         :
 *Description  :
		...
		uint16 start_block, uchar *mbr_data)
{
	...
	if (mbr_data != NULL) {
		...
		if (nand_info->boot->mbr.data.PartCount == 0) {
			...
			if (nouse == 0xffffffff) {
				...
				// erase
				PRINT("[ND]old not erase factory FirstBuild\n");
				nand_info->boot->uboot_start_block =
					get_uboot_start_block();
				nand_info->boot->uboot_next_block =
					get_uboot_next_block();
				...
				if (read_mbr_partition_v3(nand_info, chip,
							start_block) != 0) {
					PRINT("[NE]not erase MP but no mbr!\n");
					return NFTL_FAILURE;
				}
				...
				if (check_mbr_v2(nand_info,
						...
						0) {
					PRINT("[NE]not erase MP but mbr not "
							"the same 1!\n");
					return NFTL_FAILURE;
				}
			}
			...
			PRINT("[ND]new not erase factory FirstBuild\n");
			...
			if (check_mbr_v3(nand_info,
				...
				PRINT("[NE]not erase MP but mbr not the same "
						"2!\n");
				return NFTL_FAILURE;
			}
		}
		...
		if (boot->mbr.data.PartCount == 0) {
			...
			nouse =
				get_no_use_block_v3(nand_info, chip, start_block);
			if (nouse == 0xffffffff) {
				PRINT("[NE]boot but not data!\n");
				return NFTL_FAILURE;
			} else {
				PRINT("[NE]boot rebuild data!\n");
				nand_info->boot->uboot_start_block =
					get_uboot_start_block();
				nand_info->boot->uboot_next_block =
					get_uboot_next_block();
	...
	if (build_all_phy_partition_v2(nand_info) != 0) {
		PRINT("[NE]build all phy partition fail!\n");
		return NFTL_FAILURE;
	}
...
void print_boot_info(struct _boot_info *boot)
{
	...
	PRINT_DBG("[ND]boot->magic :0x%x\n", boot->magic);
	PRINT_DBG("[ND]boot->len :0x%x\n", boot->len);
	PRINT_DBG("[ND]boot->no_use_block :0x%x\n", boot->no_use_block);
	PRINT_DBG("[ND]boot->uboot_start_block :0x%x\n",
			boot->uboot_start_block);
	PRINT_DBG("[ND]boot->uboot_next_block :0x%x\n", boot->uboot_next_block);
	PRINT_DBG("[ND]boot->logic_start_block :0x%x\n",
			boot->logic_start_block);

	PRINT_DBG("[ND]mbr len :%d\n", sizeof(_MBR));
	PRINT_DBG("[ND]_PARTITION len :%d\n", sizeof(_PARTITION));
	PRINT_DBG("[ND]_NAND_STORAGE_INFO len :%d\n",
			sizeof(_NAND_STORAGE_INFO));
	PRINT_DBG("[ND]_FACTORY_BLOCK len :%d\n", sizeof(_FACTORY_BLOCK));
...
unsigned int get_no_use_block_v3(struct _nand_info *nand_info, uchar chip,
		uint16 start_block)
{
	unsigned int i, nDieNum, nBlkNum, nPage, good;
	...
	for (i = 0; i < 50; i++) {
		...
		if (nBlkNum == nand_info->BlkPerChip) {
			PRINT("[NE]1 can not find no use block !!\n");
			return 0xffffffff;
		}
	}
	return 0xffffffff;
		...
		uint16 start_block, uchar *mbr_data)
{
	...
	if (mbr_data != NULL) {
		...
		while (1) {
			...
			if (!BlockCheck(nDieNum, nBlkNum)) {
				...
				nBlkNum++;
				if (nBlkNum == nand_info->BlkPerChip) {
					PRINT("[NE]1 can not find mbr table "
							"!!\n");
					return NFTL_FAILURE;
				}
			}
	...
	while (1) {
		...
		if (!BlockCheck(nDieNum, nBlkNum)) {
			if (nand_info->FirstBuild == 1) {
				...
				if (ret == 0) {
					PRINT_DBG("[NE]get mbr_data table\n");
					break;
				} else {
	...
	PRINT_DBG("[ND]build all_phy partition start!\n");
	...
	if (nand_info->FirstBuild == 1) {
		...
		ret = read_partition_v2(nand_info,
				...
				nand_info->mbr_block_addr.Block_NO);
		if (ret != 0) {
			PRINT("[NE]read partition partition fail!\n");
			return NFTL_FAILURE;
		}
	}
		...
		uchar *mbr_data)
{
	...
	real_start_block = nand_secure_storage_first_build(block_temp);

	ret = nand_info_init_v3(nand_info, chip, real_start_block, mbr_data);
	if (ret == 0) {
		return 0;
	}
	PRINT("[NE]use old nand info init!\n");
	...
	while (1) {
		...
		if (!BlockCheck(nDieNum, nBlkNum)) {
			PHY_VirtualPageRead(nDieNum, nBlkNum, nPage, SectBitmap,
					nand_info->temp_page_buf, spare);
			if ((spare[1] == 0xaa) && (spare[2] == 0xbb)) {
				PRINT("[ND]ok get factory_bad_block table!  %d "
						"\n",
						nBlkNum);
				MEMCPY(nand_info->factory_bad_block,
						nand_info->temp_page_buf,
				...
				PRINT("[NE]read factory_bad_block table error2 "
						"%d \n",
						nBlkNum);
			}
		}
...
int write_mbr_v2(struct _nand_info *nand_info)
{
	...
	PRINT_DBG("[ND]new write mbr %d!\n", nBlkNum);

	for (i = 0; i < 10; i++) {
		ret = PHY_VirtualPageWrite(nDieNum, nBlkNum, i, SectBitmap, buf,
				spare);
		if (ret != 0) {
			PRINT("[NE]mbr_block_addr write error %d %d!\n",
					nBlkNum, i);
		}
	}

	PRINT_DBG("[ND]new write mbr end!\n");
	return 0;
}
...
int write_partition_v2(struct _nand_info *nand_info)
{
	...
	PRINT_DBG("[ND]write partition %d!\n", nBlkNum);
	...
	PRINT_DBG("[ND]write partition end!\n");
		...
		unsigned int nBlkNum)
{
	...
	PRINT_DBG("[NE]mbr partition start!\n");
	for (i = 10; i < 20; i++) {
		nPage = i;
		ret = PHY_VirtualPageRead(nDieNum, nBlkNum, nPage, SectBitmap,
				nand_info->temp_page_buf, spare);
		if ((spare[1] == 0xaa) && (spare[2] == 0xee) && (ret >= 0)) {
			PRINT_DBG("[NE]mbr partition ok!\n");
	...
	PRINT_DBG("[NE]mbr partition end!\n");
	return ret;
}
...
int write_factory_block_table_v2(struct _nand_info *nand_info)
{
	...
	PRINT("[NE]write_factory_block_table_v2!\n");
	...
	PRINT("[ND]factory bad block num:%d!\n", bad_num);
...
int write_new_block_table_v2_new(struct _nand_info *nand_info)
{
	...
	PRINT("[NE]write_new_bad_block_table new format %d!\n", nBlkNum);
	...
	for (i = 0; i < nand_info->PageNumsPerBlk; i++) {
		ret = PHY_VirtualPageWrite(nDieNum, nBlkNum, i, SectBitmap, buf,
				spare);
		if (ret != 0) {
			PRINT("[NE]bad_block_addr write error,page:%d!\n", i);
		}
	}
	return 0;
}
...
int write_new_block_table_v2(struct _nand_info *nand_info)
{
	...
	if ((spare[1] == 0xaa) && (spare[2] == 0xcc) && (spare[3] == 0xff) &&
			(spare[0] == 0xff)) {
		PRINT_DBG("[ND] new bad block table old format!\n");
		write_new_block_table_v2_old(nand_info);
	} else if ((spare[1] == 0xff) && (spare[2] == 0xff) &&
			(spare[0] == 0xff)) {
		PRINT("[ND]new bad block table old format(free block)\n");
		write_new_block_table_v2_old(nand_info);
	} else if ((spare[1] == 0xaa) && (spare[2] == 0xcc) &&
			(spare[3] == 0x01) && (spare[0] == 0xff)) {
		PRINT("[NE]new_bad_block table new format!\n");
...
int write_new_block_table_v2_new_first_build(struct _nand_info *nand_info)
{
	...
	PRINT("[NE]write_new_bad_block_table new format first build %d!\n",
			nBlkNum);
...
int write_no_use_block_v2(struct _nand_info *nand_info)
{
	...
	ret = PHY_VirtualBlockErase(nDieNum, nBlkNum);
	if (ret != 0) {
		PRINT("[NE]new_write no_use_block erase error?!\n");
	}

	PRINT_DBG("[ND]new_write_no use_block!\n");
	for (i = 0; i < nand_info->PageNumsPerBlk; i++) {
		ret = PHY_VirtualPageWrite(nDieNum, nBlkNum, i, SectBitmap, buf,
				spare);
		if (ret != 0) {
			PRINT("[NE]new_write no_use_block write error?!\n");
			return NFTL_FAILURE;
		}
	}
...
unsigned short read_new_bad_block_table_v2_old(struct _nand_info *nand_info)
{
	...
	for (i = 0; i < nand_info->PageNumsPerBlk; i++) {
		...
		if ((spare[1] == 0xaa) && (spare[2] == 0xcc) &&
			...
			num = i;
				...
				(spare[0] == 0xff)) {
			PRINT_DBG("[ND]new bad block last first use page:%d\n",
					i);
			break;
		} else {
	...
	for (i = 0; i < nDieNum; i++) {
		if (nand_info->new_bad_block[i].Chip_NO != 0xffff) {
			PRINT_DBG("[ND]new bad block:%d %d!\n",
...
unsigned short read_new_bad_block_table_v2(struct _nand_info *nand_info)
{
	...
	if ((spare[1] == 0xaa) && (spare[2] == 0xcc) && (spare[3] == 0xff) &&
		...
		page_num = read_new_bad_block_table_v2_old(nand_info);
			...
			(spare[0] == 0xff)) {
		PRINT_DBG("[ND]new bad block table old format(free block)\n");
		page_num = 0;
	} else if ((spare[1] == 0xaa) && (spare[2] == 0xcc) &&
			(spare[3] == 0x01) && (spare[0] == 0xff)) {
		PRINT_DBG("[NE]new_bad_block table new format!\n");
...
void print_nand_info_v2(struct _nand_info *nand_info)
{
	...
	PRINT_DBG("[ND]sizeof partition:%d\n", sizeof(nand_info->partition));

	for (i = 0; i < nand_info->partition_nums; i++) {
		PRINT_DBG("[ND]nand_info->partition:%d:\n", i);
		PRINT_DBG("[ND]size:0x%x\n", nand_info->partition[i].size);
		PRINT_DBG("[ND]cross_talk:0x%x\n",
				nand_info->partition[i].cross_talk);
		PRINT_DBG("[ND]attribute:0x%x\n",
				nand_info->partition[i].attribute);
		PRINT_DBG("[ND]start: chip:%d block:%d\n",
				nand_info->partition[i].start.Chip_NO,
				nand_info->partition[i].start.Block_NO);
		PRINT_DBG("[ND]end  : chip:%d block:%d\n",
...
int get_partition_v2(struct _nand_info *nand_info)
{
		...
		for (part_cnt = 0; part_cnt < mbr->PartCount &&
			...
			if ((part_type & FTL_PARTITION_TYPE) != 0) {
				...
				if ((part_type & 0x0f) == i) {
					...
					nand_info->partition[i]
						.nand_disk[m].size = mbr->array[part_cnt].len;
					nand_info->partition[i]
						.nand_disk[m].type = 0;
					MEMCPY(nand_info->partition[i]
							.nand_disk[m].name, mbr->array[part_cnt].classname, PARTITION_NAME_SIZE);
	...
	for (part_cnt = 0;
		...
		if (mbr->array[part_cnt].len == 0) {
			...
			MEMCPY(
					nand_info->partition[nand_info->partition_nums - 1].nand_disk[0].name, mbr->array[part_cnt].classname, PARTITION_NAME_SIZE);
			break;
		}
	}
#endif
...
void new_print_all_partition(struct _nand_info *nand_info)
{
	int i, m;
	for (i = 0; i < nand_info->partition_nums; i++) {
		PRINT_DBG("[NE]partition_num: %x,size :0x%x,cross_talk %x\n", i,
				nand_info->partition[i].size,
				nand_info->partition[i].cross_talk);
		...
		for (m = 0; m < MAX_PART_COUNT_PER_FTL; m++) {
			...
			PRINT_DBG("[NE]part %s size: 0x%x type: %x\n",
...
int build_all_phy_partition_v2(struct _nand_info *nand_info)
{
	...
	for (i = 0; i < nand_info->partition_nums; i++) {
		...
		if (build_phy_partition_v2(nand_info,
			...
			PRINT("[NE]build phy partition %d error!\n", i);
			return NFTL_FAILURE;
		}
	}
...
int print_mbr_data(uchar *mbr_data)
{
	...
	NAND_PARTITION *part;
	PRINT("[NE]mbr->PartCount: %d!\n", mbr->PartCount);
	for (i = 0; i < ND_MAX_PARTITION_COUNT; i++) {
		part = (NAND_PARTITION *)(&mbr->array[i]);

		PRINT("part: %d!\n", i);
		PRINT("[NE]part->name: %s !\n", part->classname);
		PRINT("[NE]part->addr: 0x%x !\n", part->addr);
		PRINT("[NE]part->len: 0x%x !\n", part->len);
		PRINT("[NE]part->user_type: 0x%x !\n", part->user_type);
		PRINT("[NE]part->keydata: %d !\n", part->keydata);
		PRINT("[NE]part->ro: %d !\n", part->ro);
		return 0;
	}
	return 0;
