...
int ndfc_soft_reset(struct _nand_controller_info *nctri)
{
	...
	PHY_DBG("Reset NDFC start %d  %x\n", nctri->channel_id, *nctri->nreg.reg_ctl);
...
s32 fill_nctri(struct _nand_controller_info *nctri)
{
	...
	nctri->nreg.reg_user_data_base = (volatile u32 *)((u8 *)reg_base + 0x0050);
	...
	nctri->nreg.reg_mbus_dma_addr = (volatile u32 *)((u8 *)reg_base + 0x00c0);
...
s32 _batch_cmd_io_send(struct _nand_controller_info *nctri, struct _nctri_cmd_seq *cmd_list)
{
	...
	*nctri->nreg.reg_sta |= *nctri->nreg.reg_sta & (NDFC_RB_B2R | NDFC_CMD_INT_FLAG | NDFC_DMA_INT_FLAG);
	...
	if ((ndfc_wait_cmdfifo_free(nctri) != 0) || (ndfc_wait_fsm_ready(nctri) != 0))  {
	print_cmd_seq(cmd_list);
	PHY_ERR("_batch_cmd io_send, wait cmd fifo free timeout!\n");
	return ERR_TIMEOUT;
	}
	 */
	//===================================
 ...
 *Note         :
 *****************************************************************************/
s32 ndfc_get_spare_data(struct _nand_controller_info *nctri, u32 *sbuf, u32 ecc_blk_cnt)
{
	s32 i;
 ...
 *Note         :
 *****************************************************************************/
s32 ndfc_set_spare_data(struct _nand_controller_info *nctri, u32 *sbuf, u32 ecc_blk_cnt)
{
	s32 i;
