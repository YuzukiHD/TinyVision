...
int m0_read_page_start(struct _nand_physic_op_par *npo)
{
	...
	if (!read_mapping_page) {
		...
		cmd_seq->nctri_cmd[0].cmd_mdata_len = ALIGN_SIZE(
				g_nssi->nsci->page_cnt_per_super_blk << 2, (1 << 10));
		if (cmd_seq->nctri_cmd[0].cmd_mdata_len <
				(8 << 9)) // 16 byte spare data in 4K
			cmd_seq->nctri_cmd[0].cmd_mdata_len = 8 << 9;
	}
...
int m0_read_page_end_not_retry(struct _nand_physic_op_par *npo)
{
	...
	// get spare data
	ndfc_get_spare_data(nci->nctri, (u32 *)spare,
			nci->sector_cnt_per_page >> nci->ecc_sector);
...
int m0_read_page_end(struct _nand_physic_op_par *npo)
{
	...
	if (ret == ECC_LIMIT) {
		PHY_ERR("read page ecc limit,chip=%d block=%d page=%d\n",
				npo->chip, npo->block, npo->page);
	} else if (ret != 0) {
		PHY_ERR("r ecc ret@%d, c@%d b@%d p@%d\n", ret, npo->chip,
				npo->block, npo->page);
	} else {
		;
	}
...
int m0_write_page_start(struct _nand_physic_op_par *npo, int plane_no)
{
	...
	if ((nci->driver_no == 1) && (nci->retry_count != 0)) {
		if ((m1_read_retry_mode == 0x4) ||
				(m1_read_retry_mode == 0x6)) {
	...
	ndfc_set_spare_data(nci->nctri, (u32 *)spare,
			nci->sector_cnt_per_page >> nci->ecc_sector);
...
int m0_write_page(struct _nand_physic_op_par *npo)
{
	...
	// PHY_ERR("%s: ch: %d  chip: %d/%d  block: %d/%d  page: %d/%d\n",
...
int m0_read_two_plane_page(struct _nand_physic_op_par *npo)
{
	...
	else if ((g_nssi->nsci->page_cnt_per_super_blk << 2) >
			(nci->sector_cnt_per_page << 9))
		ret1 = m0_read_page(&npo2);
...
int m0_bad_block_mark(struct _nand_physic_op_par *npo)
{
	...
	PHY_ERR("bad block mark chip: %d block:%d\n", npo->chip, npo->block);
	...
	if (ret) {
		...
		PHY_ERR("bad block mark, erase block failed, blk %d, chip %d, "
				"ch %d\n",
				lnpo.chip, lnpo.block, nci->nctri->channel_id);
		return ret;
	}

	MEMSET(lnpo.sdata, 0, 16);
	m0_write_page(&lnpo);
