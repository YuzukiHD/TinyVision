...
GLOBAL int ndfc_get_spare_data(struct _nand_controller_info *nctri, u32 *sbuf,
		u32 ecc_blk_cnt);
GLOBAL int ndfc_set_spare_data(struct _nand_controller_info *nctri, u32 *sbuf,
		u32 ecc_blk_cnt);
...
GLOBAL int nand_set_boot_mode(int mode);
...
GLOBAL int nand_physic_erase_super_block(unsigned int chip, unsigned int block);
...
GLOBAL int m8_write_boot0_one(unsigned char *buf, unsigned int len,
		unsigned int counter);
GLOBAL s32 m8_write_boot0_page_cfg_mode(struct _nand_chip_info *nci,
		struct _nand_physic_op_par *npo,
		struct boot_ndfc_cfg *cfg);
GLOBAL s32 m8_read_boot0_page_cfg_mode(struct _nand_chip_info *nci,
		struct _nand_physic_op_par *npo,
		struct boot_ndfc_cfg *cfg);
