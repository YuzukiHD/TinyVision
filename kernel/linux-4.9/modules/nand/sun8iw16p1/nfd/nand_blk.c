...
#define OTP_IOCTL_READ			_IO('V', 23)
#define OTP_IOCTL_WRITE			_IO('V', 24)
#define OTP_IOCTL_READ_UID		_IO('V', 25)
#define OTP_IOCTL_PROTECT		_IO('V', 26)


extern int nand_otp_storage_read(int block, int page, unsigned char *buf, unsigned int len);
extern int nand_otp_storage_write(int block, int page, unsigned char *buf, unsigned int len);
extern int nand_uid_storage_read(int block, int page, unsigned char *buf, unsigned int len);
extern int nand_otp_protect_storage_write(int block, int page, unsigned char *buf, unsigned int len);
...
*/
	...
	case OTP_IOCTL_READ:
#if 1
		nand_dbg_err("start otp read ...\n");
		down(&nandr->nand_ops_mutex);
		IS_IDLE = 0;
		if (copy_from_user(&otp_arg,
			(struct otpblc_op_t  __user *)arg,
			sizeof(otp_arg))) {
			nand_dbg_err("nand_ioctl input arg err\n");
			return -EINVAL;
		}

		nand_dbg_err("read arg block %d, page %d, len %d\n", otp_arg.block, otp_arg.page, otp_arg.len);

		buf_otp = kmalloc(otp_arg.len, GFP_KERNEL);
		if (buf_otp == NULL) {
			nand_dbg_err("buf_secure malloc fail!\n");
			return -1;
		}

		ret = nand_otp_storage_read(otp_arg.block, otp_arg.page, buf_otp, otp_arg.len);
		if (ret == 0) {
			if (copy_to_user(otp_arg.buf, buf_otp, otp_arg.len))
				ret = -EFAULT;
		} else
			ret = -EFAULT;
		kfree(buf_otp);
		...
		return ret;
#else
		ret = nand_otp_storage_read(0, 5, NULL, 2048);
		return ret;
#endif
	case OTP_IOCTL_WRITE:
		ret = 0;
		nand_dbg_err("start otp write ...\n");
		down(&nandr->nand_ops_mutex);
		IS_IDLE = 0;
		if (copy_from_user(&otp_arg,
			(struct otpblc_op_t __user *)arg,
		...
		buf_otp = kmalloc(otp_arg.len, GFP_KERNEL);
		if (buf_otp == NULL) {
			nand_dbg_err("buf_otp malloc fail!\n");
			return -1;
		}
		if (copy_from_user
		    (buf_otp, (const void *)otp_arg.buf, otp_arg.len)) {
			nand_dbg_err("buf_otp copy failed\n");
			ret = -EFAULT;
		}

		ret = nand_otp_storage_write(otp_arg.block, otp_arg.page, buf_otp, otp_arg.len);
		if (ret)
		...
		return ret;
#if 0
		ret = nand_otp_storage_write(0, 5, NULL, 2048);
#endif
		return ret;
	case OTP_IOCTL_READ_UID:
#if 1
		nand_dbg_err("start uid page read ...\n");
		down(&nandr->nand_ops_mutex);
		IS_IDLE = 0;
		if (copy_from_user(&otp_arg,
			(struct otpblc_op_t  __user *)arg,
		...
		ret = nand_uid_storage_read(otp_arg.block, otp_arg.page, buf_otp, otp_arg.len);
		if (ret == 0) {
			if (copy_to_user(otp_arg.buf, buf_otp, otp_arg.len))
				ret = -EFAULT;
		} else
		...
		return ret;
#endif
	case OTP_IOCTL_PROTECT:
		ret = 0;
		nand_dbg_err("start otp protect write ...\n");
		down(&nandr->nand_ops_mutex);
		IS_IDLE = 0;
#if 1
		if (copy_from_user(&otp_arg,
...
#endif
		ret = nand_otp_protect_storage_write(otp_arg.block, otp_arg.page, buf_otp, otp_arg.len);
		if (ret)
			printk("%s,%d %d\n", __FUNCTION__, __LINE__, ret);
		kfree(buf_otp);
		up(&(nandr->nand_ops_mutex));
