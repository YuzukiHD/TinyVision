...
extern char abnormal_nand_id[];
extern int had_write_flag;
...
s32 ndfc_get_bad_blk_flag(struct _nand_chip_info *nci, struct _nand_physic_op_par *npo,
			  u32 dis_random, u32 slen, u8 *sbuf)
{
	...
	if (dis_random) {
		...
		for (i = 0; i < len; i++) {
			_1st_spare_byte[i] =
			    sbuf[i] ^ rand_factor_1st_spare_data[i % len][num];
		}
	} else {
		for (i = 0; i < len; i++) {
 ...
 */
	 ...
	 * then retry to read, main data is bad ,and spare data is bad(all FFh)*/
	if (!memcmp(nci->npi->nand_id, abnormal_nand_id, 6)) {
		if (had_write_flag == 1) {
			/*normal condition, only a realy empty page not should come here while enable randomizationg
			 *to read, if a page which had writen, come here had explain the page has ecc err*/
			/*PHY_ERR("chip@%d b@%d p@%d had writen\n", npo->chip, npo->block, npo->page);*/
			return ERR_NO_49;
		} else {
			/*close randomization to read*/
			/*notice above to close randomization to read*/
			return ERR_NO_22;
		}
	} else {
				       ...
				       unsigned int retry_flag)
{
	...
	if (nci->randomizer) {
		if (ecc_sta == ERR_ECC) {
			...
			ret = ndfc_get_bad_blk_flag(nci, npo, nci->randomizer,
						  len, buf);
			if (ret == 0) {
				...
				MEMSET(buf, 0xff, len);
				ret_ecc_sta = 0;
			} else if (ret == ERR_NO_22) {
				ret_ecc_sta = ERR_NO_22;
			}
		}
