...
/*the raw nand flash meet some strange exception: read retry the spare is all ff,
 * actually is not all ff*/
char abnormal_nand_id[8] = {0x89, 0x88, 0x24, 0x4b, 0xa9, 0x84, 0x00, 0x00};
int had_write_flag;
...
int m0_erase_block_start(struct _nand_physic_op_par *npo)
{
	...
	nci->block_tmp = npo->block;
	nci->page_tmp = npo->page;
	nci->cmd = 3;
	return ret;
}
...
int m0_erase_block(struct _nand_physic_op_par *npo)
{
	...
	if (ret != 0) {
		PHY_ERR("erase_block wrong2, 0x%x(%d,%d)\n", npo->chip, npo->block, npo->page);
	}
...
int m0_read_page_end_not_retry(struct _nand_physic_op_par *npo)
{
	...
	/**
	 * restore the right spare data, use to compare some wrong case:
	 * JS29F16B08CCMF3(id:89h,88h,24h,4b,a9,84) meet the case:
	 * not retry to read main data is bad(some ecc block bad) and spare data is right,
	 * then retry to read, main data is bad ,but spare data is all FFh*/

	if (!memcmp(nci->npi->nand_id, abnormal_nand_id, 6)) {
		u32 ecc_block_mask = 0;
		/*U32 eblock_cnt = nci->sector_cnt_per_page >> nci->ecc_sector;*/
		U32 eblock_cnt = 0;

		if (npo->sect_bitmap > 0 && npo->sect_bitmap <= 32) {
			eblock_cnt = npo->sect_bitmap >> nci->ecc_sector;
			if (nci->nctri->channel_sel == 0) /* BCH */
				ecc_block_mask = (1 << (eblock_cnt - 1)) | ((1 << (eblock_cnt - 1)) - 1);
			else /* LDPC */
				ecc_block_mask = (1 << (eblock_cnt / 2 - 1)) |
					((1 << (eblock_cnt / 2 - 1)) - 1);
		} else {
			ecc_block_mask = 0x01;
		}
		...
		if (read_mapping_page) {
			/*in order to reduce boot times,  only read mapping page size ,
			 * so its bitmask is 0x01*/
			ecc_block_mask = 0x01;
		}
		/* if a empty page, read xk data,ecc status has x numbers bit of "1",
		 * eg. a page is 8k, read 8k data, ecc status should be 0xff,so ,we can
		 * judge an empty page by ecc status register.
		 **/
		if (nci->retry_flag == 0) {

			if (((*nci->nctri->nreg.reg_ecc_sta) & ecc_block_mask) != ecc_block_mask) {
				had_write_flag = 1;
			}
		}
	}
	...
	ret = ndfc_update_ecc_sta_and_spare_data(npo, ecc_sta, spare, nci->retry_flag);
...
int m0_write_page_start(struct _nand_physic_op_par *npo, int plane_no)
{
	...
	if ((plane_no == 0) || (plane_no == 1)) {
		...
		ret = nand_read_chip_status_ready(nci);
		if (ret)
			PHY_ERR("write page start err(%d,%d)\n", npo->block, npo->page);
	}
...
int m0_bad_block_check(struct _nand_physic_op_par *npo)
{
	...
	if (lnpo.sdata[0] != 0xff) {
		PHY_ERR("%d find a bad block: %d %d %d ", __LINE__, lnpo.chip, lnpo.block,
			lnpo.page);
	...
	for (i = 0, lnpo.page = start_page; i < num; i++) {
		m0_read_page(&lnpo);
		if (lnpo.sdata[0] != 0xff) {
			PHY_ERR("%d find a bad block: %d %d %d ", __LINE__, lnpo.chip,
