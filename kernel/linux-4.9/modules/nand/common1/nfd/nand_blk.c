...
#ifdef CONFIG_SUNXI_NAND_IOCTL_SECKV
struct seckv_op_t {
	unsigned char *key;
		      ...
		      unsigned long arg)
{
...
#endif
#ifdef CONFIG_SUNXI_NAND_IOCTL_SECKV
	struct seckv_op_t seckv_op_st;
	unsigned char *buf_key = NULL;
	unsigned char *buf_value = NULL;
	unsigned int value_len;
	int write_protect;
#endif
	...
	case SECBLK_READ:
		...
		ret = nand_secure_storage_read(sec_op_st.item, buf_secure,
					     sec_op_st.len);
	...
	case SECBLK_WRITE:
		...
		if (copy_from_user(buf_secure, (const void *)sec_op_st.buf, sec_op_st.len))
			ret = -EFAULT;
		ret = nand_secure_storage_write(sec_op_st.item, buf_secure,
					      sec_op_st.len);
		kfree(buf_secure);
		up(&(nandr->nand_ops_mutex));
		IS_IDLE = 1;
...
#ifdef CONFIG_SUNXI_NAND_IOCTL_SECKV
	case SECKV_READ:
#define MAX_VALUE_LEN (1024 * 3)
		nand_dbg_err("start secure kv read ...\n");
		down(&nandr->nand_ops_mutex);
		IS_IDLE = 0;
		buf_key = NULL;
		buf_value = NULL;

		if (copy_from_user(&seckv_op_st,
				   (struct seckv_op_t __user *)arg,
				   sizeof(seckv_op_st))) {
			nand_dbg_err("nand_ioctl input arg err\n");
			ret = -EINVAL;
			goto seckv_read_out;
		}
		buf_key = kmalloc(seckv_op_st.key_buf_len, GFP_KERNEL);
		if (!buf_key) {
			nand_dbg_err("buf_key malloc fail!\n");
			ret = -EFAULT;
			goto seckv_read_out;
		}
		if (copy_from_user(buf_key, (const void *)seckv_op_st.key,
					seckv_op_st.key_buf_len)) {
			nand_dbg_err("copy key fail!\n");
			ret = -EFAULT;
			goto seckv_read_out;
		}
		buf_key[seckv_op_st.key_buf_len] = '\0';
		buf_value = kmalloc(MAX_VALUE_LEN, GFP_KERNEL); //max payload
		if (!buf_value) {
			nand_dbg_err("buf_value malloc fail!\n");
			ret = -EFAULT;
			goto seckv_read_out;
		}

		if (sunxi_secure_storage_init() < 0) {
			nand_dbg_err("secure storage init fail!\n");
			ret = -EFAULT;
			goto seckv_read_out;
		}

		memset(buf_value, 0, MAX_VALUE_LEN);

		if (sunxi_secure_object_read(buf_key, buf_value,
					MAX_VALUE_LEN, &value_len)) {
			nand_dbg_err("secure object read fail!\n");
			ret = -EFAULT;
			goto seckv_read_out;
		}

		if (sunxi_secure_storage_exit(0) < 0) {
			nand_dbg_err("secure storage exit fail!\n");
			ret = -EFAULT;
			goto seckv_read_out;
		}

		if (copy_to_user(seckv_op_st.value, buf_value, value_len)) {
			nand_dbg_err("copy value to user fail!\n");
			ret = -EFAULT;
			goto seckv_read_out;
		}

		if (copy_to_user(seckv_op_st.value_len, &value_len, sizeof(unsigned int))) {
			nand_dbg_err("copy value_len to user fail!\n");
			ret = -EFAULT;
			goto seckv_read_out;
		}
seckv_read_out:
	...
	case SECKV_WRITE:
		nand_dbg_err("start secure kv write ...\n");
		down(&nandr->nand_ops_mutex);
		IS_IDLE = 0;
		if (copy_from_user(&seckv_op_st,
				   (struct seckv_op_t __user *)arg,
		...
		buf_key = kmalloc(seckv_op_st.key_buf_len + 1, GFP_KERNEL);
		if (!buf_key) {
			nand_dbg_err("buf_key malloc fail!\n");
			ret = -EFAULT;
			goto seckv_write_out;
		}
		if (copy_from_user(buf_key, (const void *)seckv_op_st.key,
					seckv_op_st.key_buf_len)) {
			nand_dbg_err("copy key fail!\n");
		...
		buf_key[seckv_op_st.key_buf_len] = '\0';
#ifdef DUMP_NAND_IOCTL
		print_hex_dump(KERN_CONT, "key", DUMP_PREFIX_OFFSET,
				16, 1,
				buf_key, seckv_op_st.key_buf_len, false);
#endif
		buf_value = kmalloc(seckv_op_st.value_buf_len, GFP_KERNEL);
		if (!buf_value) {
			nand_dbg_err("buf_value malloc fail!\n");
			ret = -EFAULT;
			goto seckv_write_out;
...
#ifdef DUMP_NAND_IOCTL
		print_hex_dump(KERN_CONT, "value", DUMP_PREFIX_OFFSET,
				16, 1,
				buf_value, seckv_op_st.value_buf_len, false);
#endif
		if (copy_from_user(buf_value, (const void *)seckv_op_st.value,
					seckv_op_st.value_buf_len)) {
			nand_dbg_err("copy value fail!\n");
			ret = -EFAULT;
			goto seckv_write_out;
		}
		buf_value[seckv_op_st.value_buf_len] = '\0';
		if (sunxi_secure_storage_init() < 0) {
			nand_dbg_err("secure storage init fail!\n");
			ret = -EFAULT;
			goto seckv_write_out;
		}
#ifdef CONFIG_SUNXI_NAND_IOCTL_SECKV_PROTECT
		write_protect = 1;
#else
		write_protect = 0;
#endif
		if (sunxi_secure_object_write(buf_key, buf_value,
				seckv_op_st.value_buf_len, write_protect)) {
			nand_dbg_err("secure object write fail!\n");
			ret = -EFAULT;
			goto seckv_write_out;
		}
...
seckv_write_out:
		...
		return ret;
	case SECKV_ERASE:
		nand_dbg_err("start secure kv erase ...\n");
		down(&nandr->nand_ops_mutex);
		IS_IDLE = 0;
		if (copy_from_user(&seckv_op_st,
				   (struct seckv_op_t __user *)arg,
		...
		if (sunxi_secure_object_erase_one(buf_key)) {
			nand_dbg_err("secure object erase fail!\n");
			ret = -EFAULT;
			goto seckv_erase_out;
		}
		if (sunxi_secure_storage_exit(0) < 0) {
...
#ifdef CONFIG_SUNXI_NAND_IOCTL_SECBLK
static int nand_securestorage_compat(struct block_device *bdev, fmode_t mode,
			      unsigned int cmd,
...
#ifdef CONFIG_SUNXI_NAND_IOCTL_SECKV
static int nand_securestoragekv_compat(struct block_device *bdev, fmode_t mode,
			      unsigned int cmd,
			      struct seckv_op_t32 __user *arg)
{
	struct seckv_op_t32 seckv_op32;
	struct seckv_op_t __user *seckv_op;
	int ret;

	if (copy_from_user(&seckv_op32, arg, sizeof(seckv_op32)))
		return -EFAULT;

	seckv_op = compat_alloc_user_space(sizeof(*seckv_op));
	if (!access_ok(VERIFY_WRITE, seckv_op, sizeof(*seckv_op)))
		return -EFAULT;

	if (put_user(compat_ptr(seckv_op32.key), &seckv_op->key) ||
			put_user(seckv_op32.key_buf_len,
				&seckv_op->key_buf_len) ||
			put_user(compat_ptr(seckv_op32.value),
				&seckv_op->value) ||
			put_user(seckv_op32.value_buf_len,
				&seckv_op->value_buf_len) ||
			put_user(compat_ptr(seckv_op32.value_len),
				&seckv_op->value_len))
		return -EFAULT;

	ret = nand_ioctl(bdev, mode, cmd, (unsigned long)seckv_op);
	if (ret < 0)
		return ret;
			     ...
			     unsigned int cmd, unsigned long arg)
{
	...
	case SECKV_WRITE:
	case SECKV_ERASE:
		return nand_securestoragekv_compat(bdev, mode, cmd,
						 compat_ptr(arg));
#endif
