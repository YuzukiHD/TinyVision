...
int ndfc_dma_config_start(struct nand_controller_info *nctri, u8 rw, void *buff_addr, u32 len)
{
	...
	if (nctri->dma_type == 1) {
		//MBUS DMA
		nctri->dma_addr = (unsigned long)nand_dma_map_single(&aw_ndfc, rw, buff_addr, len, 0);
		...
		config_addr = (unsigned long)nand_dma_map_single(&aw_ndfc, 1, nctri->ndfc_dma_desc_cpu, 1024, 1);
