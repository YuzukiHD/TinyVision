...
static BLOCKING_NOTIFIER_HEAD(hci_notifier_blocking);
EXPORT_SYMBOL(hci_register_notifier);

int hci_register_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&hci_notifier_blocking, nb);
}
...
{
	blocking_notifier_call_chain(&hci_notifier_blocking, event, hdev);
}
#endif
