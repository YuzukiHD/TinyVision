...
#define MELIS_LOGO_MONO		1	/* monochrome black/white */
#define MELIS_LOGO_VGA16	2	/* 16 colors VGA text palette */
#define MELIS_LOGO_CLUT224	3	/* 224 colors */
#define MELIS_LOGO_GRAY256	4	/* 256 levels grayscale */

static const char *logo_types[MELIS_LOGO_GRAY256+1] = {
    [MELIS_LOGO_MONO] = "MELIS_LOGO_MONO",
    [MELIS_LOGO_VGA16] = "MELIS_LOGO_VGA16",
    [MELIS_LOGO_CLUT224] = "MELIS_LOGO_CLUT224",
    [MELIS_LOGO_GRAY256] = "MELIS_LOGO_GRAY256"
};

#define MAX_MELIS_LOGO_COLORS	224
...
static int logo_type = MELIS_LOGO_CLUT224;
static unsigned int logo_width;
static unsigned int logo_height;
static struct color **logo_data;
static struct color logo_clut[MAX_MELIS_LOGO_COLORS];
...
static void write_footer(void)
{
    ...
    if (logo_type == MELIS_LOGO_CLUT224) {
...
static void write_logo_clut224(void)
{
	...
	for (j = 0; j < logo_width; j++) {
		...
		if (is_equal(logo_data[i][j], logo_clut[k]))
		    break;
	    if (k == logo_clutsize) {
		if (logo_clutsize == MAX_MELIS_LOGO_COLORS)
		    die("Image has more than %d colors\n"
			"Use ppmquant(1) to reduce the number of colors\n",
			MAX_MELIS_LOGO_COLORS);
...
int main(int argc, char *argv[])
{
	...
	switch (opt) {
		...
		if (!strcmp(optarg, "mono"))
		    logo_type = MELIS_LOGO_MONO;
		else if (!strcmp(optarg, "vga16"))
		    logo_type = MELIS_LOGO_VGA16;
		else if (!strcmp(optarg, "clut224"))
		    logo_type = MELIS_LOGO_CLUT224;
		else if (!strcmp(optarg, "gray256"))
		    logo_type = MELIS_LOGO_GRAY256;
		else
		    usage();
		break;
    ...
    read_image();
    switch (logo_type) {
	case MELIS_LOGO_MONO:
	    write_logo_mono();
	    break;

	case MELIS_LOGO_VGA16:
	    write_logo_vga16();
	    break;

	case MELIS_LOGO_CLUT224:
	    write_logo_clut224();
	    break;

	case MELIS_LOGO_GRAY256:
	    write_logo_gray256();
	    break;
    }
    exit(0);
