#! /usr/bin/perl
#
# checkversion find uses of MELIS_VERSION_CODE or KERNEL_VERSION
...
foreach my $file (@ARGV) {
	...
	# Look for uses: MELIS_VERSION_CODE, KERNEL_VERSION, UTS_RELEASE
	if (($_ =~ /MELIS_VERSION_CODE/) || ($_ =~ /\WKERNEL_VERSION/)) {
	    $fUseVersion = 1;
