...
die "Format: $0 [-s <systemmap-file>] <melis-file> <keyring-file>\n"
    if ($#ARGV != 1 && $#ARGV != 3 ||
	$#ARGV == 3 && $ARGV[0] ne "-s");
...
#
# Parse the melis section table
#
open FD, "objdump -h $melis |" || die $melis;
my @lines = <FD>;
close(FD) || die $melis;
...
#
# Try and parse the melis symbol table.  If the melis file has been created
# from a vmlinuz file with extract-melis then the symbol table will be empty.
#
open FD, "nm $melis 2>/dev/null |" || die $melis;
@lines = <FD>;
close(FD) || die $melis;
...
if ($nr_symbols == 0 && $sysmap ne "") {
    print "No symbols in melis, trying $sysmap\n";
...
open FD, "<$melis" || die $melis;
binmode(FD);
die $melis if (!defined(sysseek(FD, $foff, SEEK_SET)));
my $buf = "";
my $len = sysread(FD, $buf, $size);
die "$melis" if (!defined($len));
die "Short read on $melis\n" if ($len != $size);
close(FD) || die $melis;
