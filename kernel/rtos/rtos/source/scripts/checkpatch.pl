...
# avoid MELIS_VERSION_CODE
		if ($line =~ /\bMELIS_VERSION_CODE\b/) {
			WARN("MELIS_VERSION_CODE",
			     "MELIS_VERSION_CODE should be avoided, code should be for the version to which it is merged\n" . $herecurr);
		}
...
# make sure symbols are always wrapped with VMMELIS_SYMBOL() ...
# all assignments may have only one of the following with an assignment:
#	.
#	ALIGN(...)
#	VMMELIS_SYMBOL(...)
		if ($realfile eq 'melis.lds.h' && $line =~ /(?:(?:^|\s)$Ident\s*=|=\s*$Ident(?:\s|$))/) {
			WARN("MISSING_VMMELIS_SYMBOL",
			     "melis.lds.h needs VMMELIS_SYMBOL() around C-visible symbols\n" . $herecurr);
		}
