...
char *cifar10_labels[CLASS_NUM] = {"airplane", "automobile", "bird",  "cat",  "deer",
...
static void parse_output(tm_mat_t *outs) {
  ...
  TM_PRINTF("### Predict output is: Class %d, %s, prob %.3f\n", maxi,
            cifar10_labels[maxi], maxp);
  return;
}

int tinymaix_cifar10_demo(int argc, char **argv) {
  TM_DBGT_INIT();
  TM_PRINTF("cifar10 demo\n");
  tm_mdl_t mdl;

  tm_mat_t in_uint8 = {3, IMG_L, IMG_L, IMG_CH, {(mtype_t *)cifar10_pic}};
  tm_mat_t in = {3, IMG_L, IMG_L, IMG_CH, {NULL}};
  tm_mat_t outs[1];
  tm_err_t res;
  tm_stat((tm_mdlbin_t *)cifar10_mdl_data);

  res = tm_load(&mdl, cifar10_mdl_data, NULL, layer_cb, &in);
  if (res != TM_OK) {
...
FINSH_FUNCTION_EXPORT_ALIAS(tinymaix_cifar10_demo, tinymaix_cifar10_demo,
                            TinyMaix cifar10 demo)