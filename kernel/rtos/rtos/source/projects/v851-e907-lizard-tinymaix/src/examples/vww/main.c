...
char* vww_labels[CLASS_NUM] = {"no-person", "person"};
...
static void parse_output(tm_mat_t* outs)
{
    ...
    TM_PRINTF("### Predict output is: Class %d, %s, prob %.3f\n", maxi, vww_labels[maxi], maxp);
    return;
}

int tinymaix_vww_demo(int argc, char** argv)
{   TM_DBGT_INIT();
    TM_PRINTF("mnist demo\n");
    tm_mdl_t mdl;

    tm_mat_t in_uint8 = {3,IMG_L,IMG_L,IMG_CH, {(mtype_t*)vww_pic}};
...
FINSH_FUNCTION_EXPORT_ALIAS(tinymaix_vww_demo, tinymaix_vww_demo,
                            TinyMaix vww demo)