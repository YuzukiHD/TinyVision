...
{
	int ret = 0;
	rt_device_t dev = c->context;
	if (dev && dev->read) {
		ret = dev->read(dev, c->block_size * block + off, buffer, size);
	}
	return 0;
}
...
static int _lfs_flash_prog(const struct lfs_config* c, lfs_block_t block, lfs_off_t off, const void* buffer, lfs_size_t size)
{
	...
	rt_device_t dev = c->context;
	if (dev && dev->write) {
		ret = dev->write(dev, c->block_size * block + off, buffer, size);
	}
	return 0;
}
...
static int _lfs_flash_erase(const struct lfs_config* c, lfs_block_t block)
{
	...
	erase_sector.addr = c->block_size * block;
	erase_sector.len = c->block_size;
...
static void _lfs_load_config(struct lfs_config* lfs_cfg, struct rt_device* nor_device)
{
    unsigned int block_size = 4096;
    unsigned int total_size = 0;

    lfs_cfg->context = (void*)nor_device;

    lfs_cfg->read_size = LFS_READ_SIZE;
    lfs_cfg->prog_size = LFS_PROG_SIZE;

    if (nor_device && nor_device->control)
    {
        nor_device->control(nor_device, DEVICE_PART_CMD_GET_BLOCK_SIZE, &block_size);
        nor_device->control(nor_device, DEVICE_PART_CMD_GET_TOTAL_SIZE, &total_size);
    }
    ...
    lfs_cfg->block_count = total_size / block_size;
...
static _off64_t _dfs_lfs_lseek64(struct dfs_fd* file, _off64_t offset_64)
{
    dfs_lfs_fd_t* dfs_lfs_fd;
    rt_off_t offset;
    _off64_t res;
    ...
    offset = (rt_off_t)(offset_64 & 0xFFFFFFFF);
    ...
    if (file->type == FT_REGULAR)
    {
        ...
        {
            lfs_dfs_unlock();
            res = (_lfs_result_to_dfs(soff)) & 0x00000000FFFFFFFFL;
            return res;
        }
...
static int _dfs_lfs_ftruncate(struct dfs_fd* file, off_t length)
{
    dfs_lfs_fd_t* dfs_lfs_fd;
    ...
    if (file->type == FT_REGULAR)
    {
        lfs_soff_t soff = lfs_file_truncate(dfs_lfs_fd->lfs, &dfs_lfs_fd->u.file, length);
        if (soff < 0)
        {
            lfs_dfs_unlock();
            return _lfs_result_to_dfs(soff);
