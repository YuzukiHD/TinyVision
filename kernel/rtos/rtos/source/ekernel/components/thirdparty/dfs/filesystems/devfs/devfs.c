    ...
    else
    {
        ...
        if (dev_id != RT_NULL)
        {
            ...
            else if (dev_id->type == RT_Device_Class_Block)
            {
                ...
                dev_id->control(dev_id, RT_DEVICE_CTRL_BLK_GETGEOME, &geometry);
                st->st_size = geometry.bytes_per_sector * geometry.sector_count;
            }
...
static int dfs_device_fs_poll(struct dfs_fd *fd, struct rt_pollreq *req)
{
    ...
    /* get device handler */
    dev_id = (rt_device_t)fd->data;
    RT_ASSERT(dev_id != RT_NULL);

#if defined(RT_USING_POSIX)
    if (dev_id->fops && dev_id->fops->poll)
    {
        mask = dev_id->fops->poll(fd, req);
    }
#endif
    return mask;
}

static int dfs_device_fs_mmap(struct dfs_fd *fd, struct map_struct *mp)
{
    int ret = 0;
    rt_device_t dev_id;
    //software_break();
...
#if defined(RT_USING_POSIX)
    if (dev_id->fops && dev_id->fops->mmap)
    {
        ret = dev_id->fops->mmap(fd, mp);
    }
#endif
...
static int dfs_device_fs_lseek(struct dfs_fd *fd, off_t offset)
{
    int off = -1;
    rt_device_t dev_id;
...
#if defined(RT_USING_POSIX)
    if (dev_id->fops && dev_id->fops->lseek)
    {
        off = dev_id->fops->lseek(fd, offset);
    }
#endif
    return off;
}
