...
/*@{*/
static int dfs_filesystem_check_mount_flag(struct dfs_filesystem *fs, int flags)
{
    ...
    /* get rwflag when set by dfs_mount */
    rwflag = fs->rwflag;
...
int dfs_file_open(struct dfs_fd *fd, const char *path, int flags)
{
    ...
    if (dfs_filesystem_check_mount_flag(fs, flags))
    {
        rt_free(fullpath); /* release path */

        LOG_E("This is a read-only filesystem, please check your flags.");
        return -EROFS;
    }
...
int dfs_file_unlink(const char *path)
{
    ...
    if (dfs_filesystem_check_mount_flag(fs, O_RDWR))
    {
        result = -EROFS;
        LOG_E("This is a read-only filesystem, please check your flags.");
        goto __exit;
...
int dfs_file_write(struct dfs_fd *fd, const void *buf, size_t len)
{
    ...
    if (dfs_filesystem_check_mount_flag(fd->fs, fd->flags))
    {
        LOG_E("This is a read-only filesystem, please check your flags.");
        return -EROFS;
    }
 ...
 * @return the current position after seek.
 */
_off64_t dfs_file_lseek64(struct dfs_fd *fd, _off64_t offset)
{
    _off64_t result;
...
int dfs_file_rename(const char *oldpath, const char *newpath)
{
    ...
    if (dfs_filesystem_check_mount_flag(oldfs, O_RDWR))
    {
        result = -EROFS;
        LOG_E("This is a read-only filesystem, please check your flags.");
        goto __exit;
...
int dfs_file_truncate(struct dfs_fd *fd, off_t offset)
{
    int result;
    ...
    result = fd->fops->ftruncate(fd, offset);
...
#define __is_print(ch)       ((unsigned int)((ch) - ' ') < 127u - ' ')
    ...
    do
    {
        ...
        for(i = 0; i < length; i ++)
        {
            rt_snprintf(&format[i], sizeof(format) - 1, "%c", __is_print(buffer[i]) ? buffer[i] : '-');
        }
