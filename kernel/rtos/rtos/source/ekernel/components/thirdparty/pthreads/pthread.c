...
#include <log.h>
#define CONFIG_PTHREAD_USER_DATA_ID 0
...
static void _pthread_cleanup(rt_thread_t tid)
{
    ...
    ptd = (_pthread_data_t *)tid->user_data[CONFIG_PTHREAD_USER_DATA_ID];
                   ...
                   void *(*start)(void *), void *parameter)
{
    ...
    rt_snprintf(name, sizeof(name), "pth_%08x", start);
    ...
    *pid = pth_id;
    ...
    ptd->tid->user_data[CONFIG_PTHREAD_USER_DATA_ID] = (void*)ptd;
...
int pthread_timedjoin_np(pthread_t thread, void **retval, const struct timespec *abstime)
{
    ...
    uint64_t ticks = RT_WAITING_FOREVER;
    struct timeval timeval;
    uint64_t nsec;
    int64_t sec;

    ptcb = _pthread_get_data(thread);
    if (ptcb && ptcb->tid == rt_thread_self())
    {
        /* join self */
        return EDEADLK;
    }
    ...
    if (abstime != NULL) {
        ...
        ticks = sec * RT_TICK_PER_SECOND + (nsec / 1000000) / (1000 / RT_TICK_PER_SECOND);
    }
    ret = rt_sem_take(ptcb->joinable_sem, ticks);
    if (ret == RT_EOK)
    {
        /* get return value */
        if (retval != RT_NULL)
...
RTM_EXPORT(pthread_timedjoin_np);
...
pthread_t pthread_self(void)
{
    ...
    /* get pthread data from user data of thread */
    ptd = (_pthread_data_t *)rt_thread_self()->user_data[CONFIG_PTHREAD_USER_DATA_ID];
    if (ptd == NULL)
    {
        return -1;
    }
...
int pthread_cancel(pthread_t thread)
{
    ...
    /* if the thread is closed, return EINVAL.That means cancel failed! */
    if ((ptd->tid->stat & RT_THREAD_STAT_MASK) == RT_THREAD_CLOSE)
    {
        return EINVAL;
    }
...
int pthread_setname_np(pthread_t thread, const char *console_name)
{
    uint16_t name_len;
    _pthread_data_t *ptd = _pthread_get_data(thread);
    rt_thread_t thd = ptd->tid;

    int level = rt_hw_interrupt_disable();
   
    name_len = strlen(console_name);
    memset(thd->name, 0, RT_NAME_MAX);

    strncpy(thd->name, console_name, RT_NAME_MAX > name_len ? name_len : RT_NAME_MAX);
