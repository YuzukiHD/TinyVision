...
char get_header_tar_xz(archive_handle_t *archive_handle);
...
typedef struct transformer_state_t
{
    ...
    int (*xformer)(struct transformer_state_t *xstate);
...
int inflate_unzip(transformer_state_t *xstate);
int unpack_Z_stream(transformer_state_t *xstate);
int unpack_gz_stream(transformer_state_t *xstate);
int unpack_bz2_stream(transformer_state_t *xstate);
int unpack_lzma_stream(transformer_state_t *xstate);
int unpack_xz_stream(transformer_state_t *xstate);

char *append_ext(char *filename, const char *expected_ext);
int bbunpack(char **argv,
             int (*unpacker)(transformer_state_t *xstate),
...
void fork_transformer(int fd,
                      ...
                      int (*transformer)(transformer_state_t *xstate)
                     );
