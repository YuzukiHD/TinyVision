...
#define FIRST_ARGV_IS_OPT       4
    ...
    applet_opts = strcpy(malloc(strlen(applet_opts) + 1), applet_opts);
...
found_opt:
    ...
    while ((c = getopt(argc, argv, applet_opts)) != -1)
    {
        ...
        //printf("int getopt32, c = %d,  applet_opts = %s\n", c, applet_opts);
        ...
        if (optarg)
        {
            ...
            else if (on_off->param_type == PARAM_INT)
            {
                ...
                printf("xatoi_positive not define\n");
...
error:
    if (first_char != '!')
    {
        printf("first_char = %d not equal to '!' \n", first_char);
    }
    return (int32_t) -1;
}
