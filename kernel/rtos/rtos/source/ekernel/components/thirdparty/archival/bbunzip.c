...
int open_to_or_warn(int to_fd, const char *filename, int flags, int mode)
{
    ...
    printf("in open_to_or_warn, to_fd = %d\n", to_fd);
    xmove_fd(fd, to_fd);
    return 0;
}
...
int bbunpack(char **argv,
             IF_DESKTOP(long long) int (*unpacker)(transformer_state_t *xstate),
...
err:
                ...
                fd_1 = open(filename, O_RDONLY, 0);
                //printf("open old name = %s, fd_1=%d\n", filename, fd_1);
                if (fd_1 < 0)
                {
                    printf("open file:%s failed\n", filename);
                }
#else
                if (open_to_or_warn(STDIN_FILENO, filename, O_RDWR | O_CREAT, 0))
                {
                    printf("ready to go err\n");
                    goto err;
                }
#endif
            }
        ...
        else if (option_mask32 & SEAMLESS_MAGIC)
        {
        ...
        if (filename)
        {
            ...
            if (!new_name)
            {
                ...
                printf("%s: unknown suffix - ignored", filename);
                goto err;
            }
...
#if 1
            //fd_2 = open(new_name, O_RDWR | O_CREAT, stat_buf.st_mode);
            fd_2 = open(new_name, O_WRONLY | O_CREAT, stat_buf.st_mode);
            //printf("open new name = %s, fd_2=%d, st_mode=%d\n", new_name, fd_2, stat_buf.st_mode);
            if (fd_2 < 0)
            {
                printf("open file:%s failed\n", new_name);
            }
        ...
        if (option_mask32 & (OPT_STDOUT | OPT_TEST))
        {
            if (option_mask32 & OPT_TEST)
                ...
                if (open(bb_dev_null, O_WRONLY, 0))
                {
                    printf("xfunc_die  exit = %s\n", bb_dev_null);
                    xfunc_die();
                }
            filename = NULL;
        }
        ...
        if (!(option_mask32 & OPT_FORCE) && 0)
        {
            //bb_error_msg_and_die("compressed data not read from terminal, ""use -f to force it");
            printf("compressed data not read from terminal,use -f to force it \n");
        }
        ...
        if (!(option_mask32 & SEAMLESS_MAGIC))
        {
            ...
            xstate.src_fd = fd_1;
            xstate.dst_fd = fd_2;
            //printf("************xstate.src_fd = %d, xstate.dst_fd = %d, &xstatus=0x%08x\n", xstate.src_fd, xstate.dst_fd, &xstate);
...
static
char *make_new_name_gunzip(char *filename, const char *expected_ext UNUSED_PARAM)
{
    char *extension = strrchr(filename, '.');
