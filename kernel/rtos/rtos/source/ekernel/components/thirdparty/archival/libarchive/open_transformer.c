...
int check_signature16(transformer_state_t *xstate, unsigned magic16)
{
    if (!xstate->signature_skipped)
    {
        uint16_t magic2;
        //if (full_read(xstate->src_fd, &magic2, 2) != 2 || magic2 != magic16) {
        if (read(xstate->src_fd, &magic2, 2) != 2 || magic2 != magic16)
        {
...
ssize_t transformer_write(transformer_state_t *xstate, const void *buf, size_t bufsize)
{
    ssize_t nwrote;
    ...
    if (xstate->mem_output_size_max != 0)
    {
        ...
        if (size > xstate->mem_output_size_max)
        {
            ...
            printf("buffer %u too small", (unsigned)xstate->mem_output_size_max);
            nwrote = -1;
            goto ret;
        }
...
ssize_t xtransformer_write(transformer_state_t *xstate, const void *buf, size_t bufsize)
{
...
void fork_transformer(int fd,
                      ...
                      IF_DESKTOP(long long) int (*transformer)(transformer_state_t *xstate)
                     )
#else
void fork_transformer(int fd, const char *transform_prog)
#endif
{
...
static transformer_state_t *setup_transformer_on_fd(int fd, int fail_if_not_compressed)
{
    ...
    read(fd, magic.b16, sizeof(magic.b16[0]));
       ...
       )
    {
        ...
        read(fd, magic.b32, sizeof(magic.b32[0]));
    ...
    {
        //bb_error_msg_and_die("no gzip" IF_FEATURE_SEAMLESS_BZ2("/bzip2") IF_FEATURE_SEAMLESS_XZ("/xz") " magic");
        printf("bb_error_msg_and_die: no gzip\n");
    }
...
int open_zipped(const char *fname, int fail_if_not_compressed)
{
    int fd;
    transformer_state_t *xstate;
...
# if BB_MMU
    ...
    else
    {
        ...
        lseek(fd, - xstate->signature_skipped, SEEK_CUR);
        xstate->signature_skipped = 0;
    }
# else
...
void *xmalloc_open_zipped_read_close(const char *fname, size_t *maxsz_p)
{
# if 1
    transformer_state_t *xstate;
    char *image;
    ...
    else
    {
        ...
        //FIXME: avoid seek
        lseek(xstate->src_fd, - xstate->signature_skipped, SEEK_CUR);
        xstate->signature_skipped = 0;
    ...
    if (!image)
    {
        ...
        printf("read error from '%s'", fname);
    }
    close(xstate->src_fd);
    free(xstate);
    return image;
