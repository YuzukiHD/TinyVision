...
void EncAdapterPrintTopVEReg(void* pTopBaseAddr, const char* name)
{
    ...
    for(i=0;i<16;i++)
    {
        logd("%s-top-reg%02x:%08x %08x %08x %08x",name, i*16,ptr[0],ptr[1],ptr[2],ptr[3]);
        ptr += 4;
    }
    logd("\n");
}
...
void EncAdapterPrintEncReg(void* pTopBaseAddr, const char* name)
{
    ...
    for(i=0;i<16;i++)
    {
        logd("%s-enc-reg%02x:%08x %08x %08x %08x",name, i*16,ptr[0],ptr[1],ptr[2],ptr[3]);
        ptr += 4;
    }
    logd("\n");
}
...
void EncAdapterPrintIspReg(void* pTopBaseAddr, const char* name)
{
    ...
    for(i=0;i<16;i++)
    {
        logd("%s-isp-reg%02x:%08x %08x %08x %08x",name, i*16,ptr[0],ptr[1],ptr[2],ptr[3]);
        ptr += 4;
    }
    logd("\n");
}
