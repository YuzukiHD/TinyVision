...
{
    if (dr != 0)
        CDX_LOGD("%s:%ld 0x%x(%s)", file, line, (int)dr, expr);
}
...
int doLicensing(const char *la_url)
{
    ...
    int i;
    CDX_LOGD("%s %d", __func__, __LINE__);
    char *pchar;
    short *pshort;
    // chl1.dat, mode = w+b
    ...
    CDX_LOGD("%s %d result: %lx", __func__, __LINE__, oLicResponse.m_dwResult);
    if (!DRM_SUCCEEDED(oLicResponse.m_dwResult)) {
        ChkDR(DRM_E_FAIL);
    } else {
        CDX_LOGD("transid:%d", oLicResponse.m_cbTransactionID);
